...
<ComparisonTable headers={['Metric', 'Unit Tests', 'E2E Tests']} rows={[ ['Execution Speed', 'Milliseconds (extremely fast).', 'Minutes (very slow).'], ['Cost to Write/Maintain', 'Low.', 'High (UI changes frequently break tests).'], ['Confidence Level', 'Low (doesn\'t prove the app works).', 'High (proves the user can use the app).'], ['Debugging', 'Easy (points exactly to the broken line of code).', 'Hard (did the network fail? Did the DB time out?).'] ]} />
