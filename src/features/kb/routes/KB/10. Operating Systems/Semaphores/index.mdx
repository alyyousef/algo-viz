...
<ComparisonTable headers={['Property', 'Mutex', 'Semaphore']} rows={[ ['Capacity', 'Strictly binary (0 or 1). Protects a single resource.', 'Can be initialized to any number N. Protects a pool of resources.'], ['Ownership', 'Strict Ownership. The thread that locked the Mutex is the ONLY thread allowed to unlock it.', 'No Ownership. Thread A can call `wait()`, and Thread B can completely independently call `signal()` to wake it up.'], ['Primary Use Case', 'Preventing Race Conditions on shared variables.', 'Signaling between threads (Producer-Consumer problems).'] ]} />
