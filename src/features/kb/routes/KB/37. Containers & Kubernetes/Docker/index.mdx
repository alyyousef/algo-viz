...
**The stack today.** CLI to dockerd to containerd to runc to the kernel. Builds go through BuildKit. Images on disk are OCI. Hub is just the default registry hostname.
...
**Desktop versus Engine.** Engine is native on Linux. Desktop starts a Linux VM and runs Engine inside it. File shares and networking cross a VM boundary.

**What Docker is not.** It is not a cluster. Swarm exists but the industry cluster is Kubernetes. Docker is the inner loop: build, run, push.
...
**Reproducibility.** A pinned base digest plus a Dockerfile plus locked dependencies is a function from source to image digest. Unpinned latest tags are a moving input.

**Isolation model.** Docker popularized process isolation, not hardware isolation. The security model is shared kernel plus dropped caps plus seccomp, which is weaker than a hypervisor and cheaper to start.

<ComparisonTable
  headers={['Name', 'What it is', 'Where it runs']}
  rows={[
    ['Docker Engine', 'Linux daemon plus API', 'Native Linux'],
    ['Docker Desktop', 'GUI plus Linux VM plus Engine', 'Mac and Windows'],
...
That three-step loop — build, push, run — is the product thesis. Kubernetes later replaced the third step with a scheduler; it did not replace the image.
