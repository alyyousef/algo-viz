...
<ComparisonTable headers={['Aspect', 'Authentication (AuthN)', 'Authorization (AuthZ)']} rows={[ ['Purpose', 'Verifies identity (Who is this?)', 'Verifies permissions (What can they do?)'], ['Order of Operations', 'Always happens FIRST.', 'Always happens SECOND.'], ['Common Technologies', 'Passwords, MFA, Biometrics, Passkeys.', 'RBAC, ABAC, JWT Scopes, OAuth 2.0.'], ['Example Error Code', 'HTTP 401 Unauthorized (Technically means Unauthenticated).', 'HTTP 403 Forbidden.'] ]} />
