...
<ComparisonTable headers={['Feature', 'Description']} rows={[ ['Format', 'WASM is a binary format (not human readable like JS). It decodes significantly faster than JS can be parsed.'], ['Security', 'It executes in a memory-safe, sandboxed execution environment. It cannot access the DOM or the user\'s hard drive directly.'], ['Language Agnostic', 'C, C++, Rust, Go, and C# can all compile directly to WASM.'] ]} />
