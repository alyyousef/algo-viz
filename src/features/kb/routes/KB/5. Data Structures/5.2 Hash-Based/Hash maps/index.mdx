...
<ComparisonTable headers={['Operation', 'Description', 'Average Time']} rows={[ ['Put / Set', 'Inserts a new key-value pair, or overwrites the value if the key already exists.', '$O(1)$'], ['Get', 'Retrieves the value associated with a specific key. Returns null/undefined if not found.', '$O(1)$'], ['Delete / Remove', 'Removes the key-value pair from the map.', '$O(1)$'], ['Contains Key', 'Checks if a specific key exists in the map without retrieving its value.', '$O(1)$'] ]} />
