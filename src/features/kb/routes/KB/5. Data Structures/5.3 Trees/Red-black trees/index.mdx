...
<ComparisonTable headers={['Metric', 'AVL Tree', 'Red-Black Tree']} rows={[ ['Balancing Rules', 'Strict height difference of max 1.', 'Loose coloring rules (max path is 2x shortest path).'], ['Search Speed', 'Slightly faster (shorter, denser tree).', 'Slightly slower.'], ['Insert/Delete Speed', 'Slower (often requires cascading rotations up to root).', 'Significantly faster (requires max 3 rotations mathematically).'], ['Primary Use Case', 'Read-Heavy environments.', 'General purpose (Read/Write balanced).'] ]} />
