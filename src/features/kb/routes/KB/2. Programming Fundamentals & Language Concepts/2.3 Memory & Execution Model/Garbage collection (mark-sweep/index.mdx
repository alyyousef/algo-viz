...
<ComparisonTable headers={['Phase', 'Action']} rows={[ ['1. Mark', 'The GC starts at the "Roots" (global variables, active stack frames) and traverses all references, marking every object it can reach as "Alive".'], ['2. Sweep', 'The GC scans the entire Heap memory. Any object that is NOT marked as "Alive" is immediately deleted and its memory is reclaimed for future use.'] ]} />
