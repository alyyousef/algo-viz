...
<ComparisonTable headers={['Property', 'BFS (Queue)', 'DFS (Stack)'] } rows={[ ['Shortest Path?', 'YES (in unweighted graphs).', 'NO. DFS might find a path that is 100 hops long, even if a 2-hop path exists.'], ['Memory Usage', 'Massive. Must store entire levels in RAM. O(Width).', 'Minimal. Only stores the current path back to the root. O(Height).'], ['Best Use Case', 'Finding the closest node, Peer-to-Peer networks, Web Crawlers.', 'Solving Mazes, Topological Sorting, Cycle Detection, Backtracking algorithms (Sudoku).'] ]} />
