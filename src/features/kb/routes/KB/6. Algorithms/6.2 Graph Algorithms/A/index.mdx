...
<ComparisonTable headers={['Algorithm', 'Mathematical Behavior', 'Speed']} rows={[ ['Dijkstra (h=0)', 'Expands purely outward in a perfect circle (or sphere). Guarantees shortest path.', 'Very Slow (Explores massive unnecessary areas).'], ['Greedy Best-First (g=0)', 'Ignores the start distance, blindly rushes the target using only the Heuristic.', 'Blazingly Fast, but often gets trapped by walls and returns terrible, sub-optimal paths.'], ['A* (g + h)', 'Mathematically balances both. Guarantees the absolute shortest path while minimizing wasted exploration.', 'Fast and Perfect.'] ]} />
