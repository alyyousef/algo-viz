...
<ComparisonTable headers={['Metric', 'Complexity', 'Explanation']} rows={[ ['Time (Worst/Avg/Best)', 'O(n log n)', 'It ALWAYS divides the array in half (log n), and ALWAYS merges them (n).'], ['Space', 'O(n)', 'Requires a temporary array of the exact same size as the input to perform the merge.'], ['Stable?', 'Yes', 'Maintains relative order of equal elements.'] ]} />
