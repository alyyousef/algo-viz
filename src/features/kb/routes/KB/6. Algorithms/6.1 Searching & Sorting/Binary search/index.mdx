...
<ComparisonTable headers={['Case', 'Time Complexity', 'Explanation']} rows={[ ['Best Case', 'O(1)', 'The target is exactly in the middle on the very first check.'], ['Average Case', 'O(log n)', 'The array is repeatedly halved.'], ['Worst Case', 'O(log n)', 'The target is at the very ends of the array, or not present at all.'] ]} />
