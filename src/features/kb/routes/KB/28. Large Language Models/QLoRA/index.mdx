...
<ComparisonTable headers={['Innovation', 'Description']} rows={[ ['4-bit NormalFloat (NF4)', 'A mathematically optimal data type designed specifically for the normal distribution of neural network weights, retaining far more accuracy than standard 4-bit integers.'], ['Double Quantization', 'Quantizing the quantization constants themselves, saving an additional 0.37 bits per parameter (which equals 3GB of VRAM on a 65B model).'], ['Paged Optimizers', 'Using Nvidia Unified Memory to mathematically page optimizer states to CPU RAM if the GPU VRAM runs out, preventing catastrophic Out Of Memory (OOM) crashes.'] ]} />
