...
<ComparisonTable headers={['Advantage', 'Description']} rows={[ ['Single File Deployment', 'A GGUF file contains the entire model: the weights, the tokenizer vocabulary, and all hyperparameter metadata. No external configuration files are needed.'], ['Extensibility', 'It is a key-value based format. If a new architecture requires a new metadata field, it can be added to the GGUF file without breaking older software that parses it.'], ['mmap Capability', 'The file can be mathematically memory-mapped directly from the SSD to RAM, meaning loading a 40GB model takes seconds instead of minutes.'] ]} />
