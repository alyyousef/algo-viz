...
Historically, playing audio on the web meant using the `<audio>` HTML tag. This is fine for playing a podcast, but it is mathematically useless for building a video game or a Digital Audio Workstation (DAW) like GarageBand. The `<audio>` tag suffers from high latency, cannot mix multiple tracks simultaneously, and cannot apply real-time mathematical effects like Reverb or EQ.
...
1. **Source Nodes:** Where the sound begins. This can be an MP3 file (`AudioBufferSourceNode`), your laptop's microphone (`MediaStreamAudioSourceNode`), or a mathematically generated sine wave (`OscillatorNode`).
2. **Processing Nodes:** The mathematical filters. You pass the source through a Volume control (`GainNode`), an Equalizer (`BiquadFilterNode`), or a Spatializer (`PannerNode`) to make the sound appear as if it's behind your left ear.
3. **Destination Node:** The final output. Usually `audioCtx.destination`, which mathematically maps directly to your physical laptop speakers or headphones.
...
If you attempt to do this in standard JavaScript using a `while` loop, the V8 garbage collector will eventually pause your code for 5 milliseconds, causing the audio to audibly crackle and pop (buffer underrun).

The Web Audio API solves this by moving all the heavy DSP mathematics into a dedicated background C++ thread. You define the graph in JavaScript, but the actual 44.1 kHz mathematical crunching happens in highly optimized C++ (or WebAssembly via `AudioWorklet`), guaranteeing zero crackling even if the main UI thread freezes.
...
**A:** Spatial audio mathematically tricks the human brain into hearing sound in 3D space using standard stereo headphones. The `PannerNode` uses HRTF (Head-Related Transfer Functions) mathematics. If you program an explosion at coordinates `(x: 10, y: 0, z: 0)` (far to the right), the API mathematically delays the sound reaching the left ear by a fraction of a millisecond and muffles the high frequencies (simulating the physical shadow of the human head), creating a perfect 3D illusion.

**Q: Why does `audioCtx.resume()` exist? (The Autoplay Policy problem)**
**A:** Historically, advertisers abused the Web Audio API to blast audio the millisecond a webpage loaded. In 2018, Google mathematically broke this by enforcing the **Autoplay Policy**. When a page loads, the `AudioContext` starts in a "suspended" state. It mathematically cannot make sound until the user explicitly interacts with the DOM (a click or a tap). The developer must hook into that click event and execute `audioCtx.resume()` to unlock the audio thread.

**Q: What is an `AudioWorklet`?**
**A:** The standard API provides pre-built nodes (like Reverb or EQ). But what if you want to build a custom distortion pedal with entirely new mathematics? Historically, developers used `ScriptProcessorNode`, which executed custom JS on the main thread (causing terrible crackling). **AudioWorklet** is the modern replacement. It allows you to write custom DSP math (in JS or highly optimized WebAssembly) and inject it *directly* into the browser's dedicated C++ audio rendering thread, ensuring perfect, glitch-free performance.
