...
<ComparisonTable headers={['Type', 'Description']} rows={[ ['Subtype (Runtime)', 'Method Overriding. A child class replaces the implementation of a parent class method.'], ['Ad-hoc (Compile-time)', 'Method Overloading. Having three functions named `add()`, but one takes ints, one takes floats, one takes strings.'], ['Parametric', 'Generics. Writing a single `List<T> ` class that works for any type T.'] ]} />
