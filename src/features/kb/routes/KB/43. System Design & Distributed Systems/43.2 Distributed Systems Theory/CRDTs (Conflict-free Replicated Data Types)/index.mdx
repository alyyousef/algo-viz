...
<ComparisonTable headers={['CRDT Type', 'How it Works', 'Example']} rows={[ ['Grow-Only Counter (G-Counter)', 'A distributed counter where nodes can only ADD, never subtract. Every node mathematically tracks its own count, and the total is the MAX of all nodes.', 'A YouTube Video View Counter.'], ['Positive-Negative Counter (PN-Counter)', 'A combination of two G-Counters (one for Adds, one for Subtracts). The final value is the mathematical difference between the two.', 'A Reddit Upvote/Downvote system.'], ['Observed-Remove Set (OR-Set)', 'A set where items can be added and removed. Every element is tagged with a mathematically unique UUID upon insertion to avoid deletion conflicts.', 'A distributed Shopping Cart.'] ]} />
