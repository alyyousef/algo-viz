...
<ComparisonTable headers={['Asset Type', 'Should you CDN it?', 'Explanation']} rows={[ ['Static Assets (Images, Video, CSS, JS)', 'Yes (Mandatory)', 'These files never mathematically change per user. Serving them from the Origin server is a massive waste of expensive CPU and bandwidth.'], ['HTML Pages', 'Sometimes', 'If it is a public blog post, cache it! If it is a logged-in user\'s dashboard, it cannot be cached globally.'], ['API Responses (JSON)', 'Rarely', 'Most API data is highly dynamic and user-specific. However, a public API like a weather forecast should absolutely be CDN-cached for a few minutes.'] ]} />
