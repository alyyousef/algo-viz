...
<ComparisonTable headers={['Component', 'Mathematical Role', 'Example']} rows={[ ['Topic', 'The central named channel where messages are broadcast.', '`topic/user_signups`'], ['Publisher', 'The system that mathematically generates the data and fires it into the topic.', 'The Frontend Web Server.'], ['Subscriber', 'The system that registers a mathematical interest in a topic. Every subscriber gets a perfect clone of every message.', 'The Welcome Email microservice.'] ]} />
