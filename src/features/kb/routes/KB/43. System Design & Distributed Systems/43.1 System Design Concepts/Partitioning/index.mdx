...
<ComparisonTable headers={['Type', 'Mathematical Mechanism', 'Use Case']} rows={[ ['Horizontal Partitioning', 'Splitting rows. (e.g., Table 1 holds rows 1-1M, Table 2 holds rows 1M-2M).', 'Managing massive time-series data or log files.'], ['Vertical Partitioning', 'Splitting columns. Moving the massive, rarely-accessed `UserBioText` column into a separate table, leaving the heavily accessed `Username` and `PasswordHash` in the primary table.', 'Optimizing RAM cache. The database can fit way more Usernames into memory if it doesn\'t have to load the Bio text.'] ]} />
