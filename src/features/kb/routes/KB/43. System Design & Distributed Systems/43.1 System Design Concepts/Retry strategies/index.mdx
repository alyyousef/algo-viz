...
<ComparisonTable headers={['Pattern', 'Mathematical Formula', 'Effect']} rows={[ ['Exponential Backoff', 'Wait `2^attempt * base_delay`. (e.g., 1s, 2s, 4s, 8s, 16s).', 'Gives the struggling backend server mathematically increasing amounts of time to recover before hitting it again.'], ['Jitter (Randomization)', 'Wait `ExponentialBackoff + Random(-1s to 1s)`.', 'Mathematically de-synchronizes the clients. Instead of 10,000 clients all retrying at exactly 2.000 seconds, they retry scattered randomly between 1.0 and 3.0 seconds, smoothing the load.'], ['Idempotency Keys', 'Send a unique UUID with the request.', 'Ensures that if a retry is sent for a credit card charge, the server mathematically guarantees it will not charge the card twice.'] ]} />
