...
<ComparisonTable headers={['Failure Layer', 'Fault Tolerant Design Pattern']} rows={[ ['Hard Drive Failure', 'RAID (Redundant Array of Independent Disks). Data is mathematically striped or mirrored across multiple physical disks inside the same server.'], ['Server Failure', 'Load Balancing. If the EC2 instance dies, the Load Balancer instantly detects the mathematical loss of heartbeat and routes traffic to healthy instances in the cluster.'], ['Datacenter Failure', 'Multi-AZ (Availability Zone). Replicating the database synchronously across three physically separate buildings powered by different municipal power grids.'], ['Regional Disaster (Earthquake)', 'Multi-Region Active-Passive or Active-Active deployment (e.g., US-East routing traffic to EU-West via global DNS failover).'] ]} />
