...
<ComparisonTable headers={['Exchange Type', 'Routing Behavior']} rows={[ ['Direct', 'Routes messages to a queue whose binding key exactly matches the routing key of the message.'], ['Fanout', 'Broadcasts all messages to all connected queues, ignoring the routing key.'], ['Topic', 'Routes messages based on wildcard matching between the routing key and the queue binding (e.g., `logs.*.error`).'], ['Headers', 'Routes based on message header attributes instead of the routing key.'] ]} />
