...
<ComparisonTable headers={['Class', 'Mathematical Definition', 'Examples']} rows={[ ['P (Polynomial Time)', 'Algorithms that scale mathematically in `O(n^k)`. They are considered "fast" and efficiently solvable by modern computers.', 'Sorting algorithms, Shortest Path (Dijkstra), Hash Map lookups.'], ['NP (Nondeterministic Polynomial)', 'If you are mathematically handed the correct answer, you can *verify* it is correct in Polynomial Time (`P`).', 'Sudoku. Solving a massive Sudoku board is brutally hard. But if someone hands you the completed board, verifying no numbers repeat takes 2 seconds.'], ['NP-Complete', 'The mathematically hardest problems in `NP`. If you find a fast algorithm for one NP-Complete problem, you instantly solve ALL of them.', 'The Traveling Salesperson Problem, Knapsack Problem, Boolean Satisfiability (SAT).'] ]} />
