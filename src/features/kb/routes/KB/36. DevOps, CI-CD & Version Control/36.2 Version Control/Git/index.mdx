...
<ComparisonTable headers={['Area', 'Description', 'Command to Move To']} rows={[ ['Working Directory', 'The actual files you see and edit on your hard drive.', '(Default state)'], ['Staging Area (Index)', 'A middle-ground where you prepare and group your modified files before committing them.', `git add <file> `], ['Local Repository', 'The `.git` folder where Git permanently stores the frozen snapshots (commits) of your project.', `git commit`] ]} />
