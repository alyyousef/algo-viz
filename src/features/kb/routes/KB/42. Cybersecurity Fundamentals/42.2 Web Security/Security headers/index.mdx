...
<ComparisonTable headers={['Header', 'Purpose']} rows={[ ['Strict-Transport-Security (HSTS)', 'Mathematically forces the browser to ONLY ever communicate with the domain over HTTPS, permanently preventing SSL-Stripping MITM attacks.'], ['Content-Security-Policy (CSP)', 'The ultimate defense against XSS. Tells the browser exactly which domains are allowed to load JavaScript, preventing attackers from executing inline scripts.'], ['X-Frame-Options', 'Defends against Clickjacking by mathematically telling the browser that this website is never allowed to be rendered inside an `<iframe> ` on a malicious site.'], ['X-Content-Type-Options', 'Set to `nosniff`. Prevents the browser from mathematically guessing the MIME type of a file, stopping attackers from disguising a malicious HTML file as an innocent image upload.'] ]} />
