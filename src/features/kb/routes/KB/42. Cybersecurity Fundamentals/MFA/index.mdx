...
<ComparisonTable headers={['Method', 'Security Level', 'Vulnerabilities']} rows={[ ['SMS Text Messages', 'Low', 'Highly vulnerable to SIM Swapping attacks (where the hacker tricks the telecom company into porting your phone number to their SIM card).'], ['Authenticator Apps (TOTP)', 'Medium', 'Apps like Google Authenticator mathematically generate a 6-digit code every 30 seconds. Vulnerable to advanced phishing (Adversary-in-the-Middle), where the fake website asks the user to type in the 6-digit code.'], ['Hardware Security Keys (FIDO2)', 'Maximum', 'Physical USB keys like a YubiKey. They mathematically bind the authentication challenge to the specific TLS certificate of the website. They are cryptographically immune to phishing.'] ]} />
