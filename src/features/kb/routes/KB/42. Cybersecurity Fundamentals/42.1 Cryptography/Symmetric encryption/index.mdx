...
<ComparisonTable headers={['Cipher Type', 'Mechanism', 'Primary Use Case']} rows={[ ['Block Cipher', 'Mathematically chunks data into fixed-size blocks (e.g., 128 bits) and encrypts each block sequentially using complex substitution and permutation networks.', 'Database encryption, file encryption, TLS (AES-GCM).'], ['Stream Cipher', 'Mathematically generates an infinite pseudo-random keystream based on the secret key. It then applies a bitwise XOR (`^`) between the plaintext and the keystream.', 'Real-time hardware encryption, ultra-low latency protocols (ChaCha20).'] ]} />
