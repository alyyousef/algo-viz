...
<ComparisonTable headers={['Feature', 'Asymmetric Encryption', 'Symmetric Encryption']} rows={[ ['Keys', 'Two (Public and Private)', 'One (Shared Secret)'], ['Speed', 'Extremely Slow (Heavy Math)', 'Extremely Fast (Bitwise operations)'], ['Primary Use Case', 'Key Exchange, Digital Signatures (RSA, ECC)', 'Bulk Data Encryption (AES, ChaCha20)'], ['Key Exchange Problem', 'Solved (Public keys are public)', 'Fatal (Must securely share the key)'] ]} />
