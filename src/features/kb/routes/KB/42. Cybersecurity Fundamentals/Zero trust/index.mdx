...
<ComparisonTable headers={['Feature', 'Legacy VPN', 'Zero Trust (ZTNA)']} rows={[ ['Access Model', 'Connects the user to the entire corporate network.', 'Connects the user strictly to a specific application.'], ['Trust', 'Implicit. Once authenticated to the VPN, you are trusted.', 'Continuous. Every single HTTP request is cryptographically re-evaluated.'], ['Visibility', 'Network-level IP addresses.', 'Application-level identity and device context.'] ]} />
