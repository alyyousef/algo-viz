...
<ComparisonTable headers={['Paradigm', 'Mechanism']} rows={[ ['Single Sign-On (SSO)', 'The user authenticates once against a central mathematical Identity Provider (IdP) like Okta. The IdP then issues trusted cryptographic tokens (SAML/OIDC) to automatically log the user into all other corporate apps.'], ['Passwordless Authentication', 'Replacing passwords entirely with cryptographic possession. The user relies on their device\'s secure enclave (like Apple FaceID or Windows Hello) to mathematically sign challenges from the server (WebAuthn/FIDO2).'], ['Machine-to-Machine (M2M)', 'Servers talking to servers. Uses mathematical mechanisms like Mutual TLS (mTLS) or API Keys rather than human passwords.'] ]} />
