...
<ComparisonTable headers={['Capability', 'Description']} rows={[ ['Signature-Based Detection', 'Inspects live network traffic against thousands of known mathematical rules and malware signatures to instantly drop malicious packets.'], ['Protocol Identification', 'Automatically identifies protocols (HTTP, DNS, TLS) on ANY port, defeating malware that tries to hide SSH traffic on port 80.'], ['File Extraction', 'Can automatically extract files (like PDFs or EXEs) out of the raw network stream and save them to disk for isolated malware analysis.'] ]} />
