...
<ComparisonTable headers={['Scan Type', 'Description', 'Accuracy']} rows={[ ['Uncredentialed Scan', 'Scans the machine from the outside over the network. It can only see what a hacker would see (open ports, banner grabbing).', 'Low. Prone to false positives because it cannot see inside the OS.'], ['Credentialed Scan', 'The scanner is given an Administrator/Root password. It logs into the machine via SSH/RDP and mathematically inspects the registry, local files, and installed packages.', 'Extremely High. It knows exactly what is vulnerable.'] ]} />
