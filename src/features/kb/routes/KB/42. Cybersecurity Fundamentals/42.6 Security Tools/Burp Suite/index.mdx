...
<ComparisonTable headers={['Tool', 'Function']} rows={[ ['Proxy', 'The core feature. Intercepts, inspects, and modifies HTTP/HTTPS traffic in real-time.'], ['Repeater', 'Allows a hacker to take a captured HTTP request, tweak a specific parameter, and manually resend it to the server hundreds of times to test how the server reacts to malformed math or logic.'], ['Intruder', 'A highly customizable brute-forcer. You define a payload position in the HTTP request (e.g., the `id=123` parameter) and Intruder automatically fires thousands of requests substituting payloads from a dictionary to find vulnerabilities.'] ]} />
