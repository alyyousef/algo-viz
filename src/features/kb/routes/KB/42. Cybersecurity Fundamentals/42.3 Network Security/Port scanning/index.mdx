...
<ComparisonTable headers={['State', 'Meaning', 'Hacker Interpretation']} rows={[ ['Open', 'The server responded with an ACK. An application is actively listening on this port.', 'This is the target. I must figure out if the application listening here has a vulnerability.'], ['Closed', 'The server responded with an RST (Reset). No application is listening.', 'Move on, nothing to hack here.'], ['Filtered', 'The server did not respond at all. The packet was dropped into a black hole.', 'There is a firewall blocking my scan. I cannot mathematically determine if the port is open or closed.'] ]} />
