...
<ComparisonTable headers={['Layer Type', 'Function']} rows={[ ['Input Layer', 'Passively receives the raw features (e.g., pixel intensities of an image).'], ['Hidden Layers', 'The computational engine. Each neuron here learns to detect increasingly abstract features. The "Deep" in Deep Learning refers to having many hidden layers.'], ['Output Layer', 'Produces the final prediction (e.g., probability of "Cat" vs "Dog").'] ]} />
