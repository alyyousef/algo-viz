...
<ComparisonTable headers={['Type', 'Who it protects', 'Use Case']} rows={[ ['Forward Proxy', 'The Client (User).', 'A corporate VPN proxy that hides the employee\'s IP address from the websites they visit.'], ['Reverse Proxy', 'The Server (Backend).', 'Nginx hiding your internal database and application servers from the public internet.'] ]} />
