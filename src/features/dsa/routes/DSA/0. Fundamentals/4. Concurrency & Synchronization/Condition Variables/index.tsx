...
const bigPicture = [
  ...
  },
  {
    ...
    notes: 'Understanding spurious wakeups and lost wakeups is essential to safe parallel code.',
  },
]
...
const mentalModel = [
  {
    title: 'Predicate',
    detail: 'A boolean expression over shared state. Example: bufferCount > 0.',
  },
  {
    title: 'Wait queue',
    detail: 'Threads that cannot proceed park on the condition variable while holding no locks.',
  },
  {
    title: 'Signal',
    detail: 'A thread changes state so the predicate may become true, then notifies waiters.',
  },
  {
    title: 'Re-check',
    detail: 'Woken threads re-acquire the mutex and re-test the predicate before continuing.',
  },
]
...
const glossary = [
  {
    ...
    definition: 'A synchronization object for waiting on a predicate protected by a mutex.',
  },
  {
    term: 'Predicate',
    definition: 'The exact condition that must hold before a thread may proceed.',
  },
  {
    term: 'wait()',
    definition: 'Atomically releases the mutex and sleeps, then re-acquires it before returning.',
  },
  {
    term: 'signal()/notify()',
    definition: 'Wake one waiting thread (if any). It does not transfer the mutex.',
  },
  {
    term: 'broadcast()/notifyAll()',
    definition: 'Wake all waiting threads; each re-checks the predicate.',
  },
  {
    term: 'Mesa semantics',
    definition: 'Woken threads must re-check the predicate; the signaller keeps the lock.',
  },
  {
    term: 'Hoare semantics',
    definition: 'Signaller yields the lock immediately to a woken waiter (less common).',
  },
  { term: 'Spurious wakeup', definition: 'A wait returns without the predicate becoming true.' },
  {
    term: 'Lost wakeup',
    definition: 'A signal happens before a thread begins waiting, so the waiter sleeps forever.',
  },
  {
    term: 'Monitor',
    definition: 'An object that bundles state, a mutex, and one or more condition variables.',
  },
]
...
const coreRules = [
  ...
  },
  {
    ...
    detail: 'First mutate the shared state under the mutex, then signal/broadcast.',
  },
  {
...
const apiSurface = [
  {
    title: 'wait(mutex)',
    detail: 'Atomically unlocks the mutex and sleeps. When it returns, the mutex is locked again.',
  },
  {
    title: 'signal() / notify_one()',
    detail: 'Wakes a single waiter if one exists; otherwise does nothing.',
  },
  {
    title: 'broadcast() / notify_all()',
    detail: 'Wakes all waiters so they can race to re-check the predicate.',
  },
  {
    title: 'timed_wait(deadline)',
    detail: 'Sleeps until signaled or until a timeout. Still requires a predicate loop.',
  },
]
...
const semanticsComparison = [
  ...
  },
  {
    ...
    detail: 'Even with Mesa semantics, correct code is simple: always wait in a while-loop.',
  },
]
...
const keyPatterns = [
  ...
  },
  {
    ...
    detail: 'Threads wait until a counter reaches N, then all are released together.',
  },
  {
    title: 'Thread pool work queue',
    detail: 'Workers sleep on notEmpty; submitters signal after enqueuing work.',
  },
  {
    title: 'Readers-writers',
    detail: 'Readers wait when a writer is active; writers wait until readers drop to zero.',
  },
  {
    title: 'One-time latch',
    detail: 'Threads wait until a flag becomes true; a single broadcast releases all.',
  },
  {
    title: 'Rate limiting',
    detail: 'Waiters sleep until tokens replenish; signallers broadcast after refill.',
  },
]
...
const correctnessChecklist = [
  ...
  },
  {
    ...
    detail: 'Ensure that signals eventually reach waiters and that no thread can wait forever.',
  },
  {
  ...
  {
    title: 'Shutdown path',
    detail: 'Define how to wake and exit threads when the system is stopping.',
  },
]
...
const commonPitfalls = [
  {
    ...
    description: 'You can miss a signal if the predicate changes before you start waiting.',
  },
  {
  ...
  },
  {
    ...
    description: 'Wakes too many threads, causing the thundering herd effect and wasted CPU.',
  },
  {
  ...
  },
  {
    ...
    description: 'Threads can wait forever unless there is a clear termination predicate.',
  },
]
...
const implementationNotes = [
  ...
  },
  {
    ...
    detail: 'wait() must release the mutex and park atomically to avoid missing notifications.',
  },
  {
    title: 'Spurious wakeups are allowed',
    detail: 'Many systems allow spurious wakes for efficiency; correct code must tolerate them.',
  },
  {
...
const performanceTips = [
  ...
  },
  {
    ...
    detail: 'Shorter lock holds reduce convoying and increase throughput.',
  },
  {
    title: 'Batch work before signaling',
    detail: 'Enqueue multiple items and then signal once if a single wakeup can drain the queue.',
  },
  {
    title: 'Use monotonic clocks for timed waits',
    detail: 'Avoid timeouts that shift with system clock changes.',
  },
]
...
notEmpty.signal()
unlock(mutex)`,
    explanation: 'Signal after changing state. The consumer still re-checks.',
  },
  {
...
notFull.signal()
unlock(mutex)`,
    explanation: 'Two condition variables keep producer and consumer roles separate.',
  },
]
...
const advancedPatterns = [
  {
    ...
    detail: 'Separate a complex predicate into smaller ones with dedicated condition variables.',
  },
  {
    title: 'Sequence numbers',
    detail: 'Use counters to avoid lost wakeups when waiters check for a specific event.',
  },
  {
    title: 'Two-phase shutdown',
    detail: 'Set a shutdown flag, broadcast, and let waiters exit gracefully.',
  },
  {
    title: 'Mesa-style guard objects',
    detail: 'Wrap wait in a helper function to enforce the loop and reduce mistakes.',
  },
]
...
const signalChoiceGuide = [
  {
    ...
    detail: 'Example: a bounded buffer with capacity > 0 where a single enqueue frees one slot.',
  },
  {
    title: 'Broadcast when many waiters may proceed',
    detail: 'Example: a latch flips from false to true; all waiters should wake.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Broadcast can cause many wakeups that immediately go back to sleep; use sparingly.',
  },
]
...
const lifecycleSteps = [
  {
    ...
    detail: 'Decide the exact condition a waiter needs, e.g., count > 0 or shutdown == true.',
  },
  {
    title: '2) Protect state',
    detail: 'Guard all predicate-related state with a mutex.',
  },
  {
    title: '3) Wait in loop',
    detail: 'While predicate is false, wait. The wait releases and later re-acquires the mutex.',
  },
  {
    title: '4) Update + signal',
    detail: 'When a thread changes state so the predicate may become true, signal or broadcast.',
  },
  {
    title: '5) Re-check and proceed',
    detail: 'A woken thread re-tests the predicate and proceeds only if it is true.',
  },
]
...
const debuggingChecklist = [
  {
    ...
    detail: 'Ensure every read/write of predicate state is under the same mutex.',
  },
  {
    title: 'Are waits in a loop?',
    detail: 'Check for if(condition) wait(...) which can break under spurious wakeups.',
  },
  {
    title: 'Are signals after updates?',
    detail: 'Signals before state changes can wake threads too early.',
  },
  {
    title: 'Are the right condition variables used?',
    detail: 'One condition variable per predicate keeps signaling precise.',
  },
  {
    title: 'Is shutdown handled?',
    detail: 'Add a termination predicate and broadcast so threads can exit.',
  },
  {
    title: 'Are timeouts handled like spurious wakes?',
    detail: 'Timed waits must still re-check the predicate.',
  },
]
...
const faq = [
  ...
  },
  {
    ...
    answer: 'Polling wastes CPU and can miss updates; condition variables avoid both issues.',
  },
  {
...
const exampleTable = [
  {
    ...
    predicate: 'count > 0 / count < capacity',
    signal: 'signal opposite side',
  },
...
unlock(mutex)
// proceed`,
    explanation: 'Any number of threads can wait until a single event flips ready to true.',
  },
  {
...
const timelineScenarios = [
  {
    ...
    summary: 'The mutex + predicate + while-loop guarantees correctness even with contention.',
  },
  {
    id: 'lost-wakeup',
    ...
    summary: 'Not holding the mutex during check + wait makes missed signals possible.',
  },
  {
    id: 'spurious',
    ...
    summary: 'Spurious wakeups are harmless if you use a while-loop.',
  },
]
...
export default function ConditionVariablesPage(): JSX.Element {
  const { activeTab, setActiveTab } = useTopicTabs({
    tabs,
    pageTitle: 'Condition Variables',
    defaultTab: 'big-picture',
  })

  const [selectedScenarioId, setSelectedScenarioId] = useState(
    timelineScenarios[0]?.id ?? 'correct',
  )
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Condition Variables</h1>
      <p>
        Condition variables are the standard tool for blocking threads until a shared predicate
        becomes true. They always work with a mutex: the mutex protects the predicate, and the
        condition variable handles sleeping and waking.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          ...
          <section id="bp-why" className="bin98-section">
            ...
            <p>
              Correctness with condition variables requires a predicate, a mutex, and a loop.
              Notifications are hints, not guarantees.
            </p>
            <p>
              Most practical patterns, including bounded buffers, barriers, and readers-writers
              coordination, rely on this precise contract.
            </p>
          </section>
          <hr className="bin98-divider" />
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-rules" className="bin98-section">
            ...
            <p>
              The condition variable does not store the condition. The predicate lives in shared
              state. The condition variable only manages sleep and wake coordination.
            </p>
          </section>
          <section id="core-api" className="bin98-section">
            <h2 className="bin98-heading">API Surface</h2>
            {apiSurface.map((item) => (
              <p key={item.title}>
                <strong>{item.title}:</strong> {item.detail}
              </p>
          ...
          <section id="core-patterns" className="bin98-section">
            <h2 className="bin98-heading">Patterns You Will Reuse</h2>
          ...
          <section id="core-advanced" className="bin98-section">
            <h2 className="bin98-heading">Advanced Patterns</h2>
          ...
          <section id="core-signal" className="bin98-section">
            <h2 className="bin98-heading">Signal vs Broadcast Guide</h2>
          ...
          <section id="core-lifecycle" className="bin98-section">
            <h2 className="bin98-heading">Lifecycle Checklist</h2>
      ...
      {activeTab === 'examples' && (
        <>
          ...
          <section id="ex-timeline" className="bin98-section">
            ...
            <p>
              Pick a scenario, then step through the timeline. This highlights why correct usage
              always pairs a predicate with a mutex and a while-loop around wait.
            </p>
          ...
          <section id="ex-patterns" className="bin98-section">
            <h2 className="bin98-heading">Patterns at a Glance</h2>
            {exampleTable.map((row) => (
              <p key={row.pattern}>
                <strong>{row.pattern}:</strong> Predicate `{row.predicate}`; typical signal `
                {row.signal}`.
              </p>
            ))}
          </section>
