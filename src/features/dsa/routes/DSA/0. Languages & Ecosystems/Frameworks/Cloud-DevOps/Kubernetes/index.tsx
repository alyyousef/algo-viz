...
const glossarySections: GlossarySection[] = [
  {
    ...
    terms: [
      { term: 'Cluster', definition: 'A set of control plane and worker nodes managed as one Kubernetes system.' },
      { term: 'API server', definition: 'The central API endpoint for reading and writing Kubernetes resources.' },
      { term: 'etcd', definition: 'The backing key-value store that persists Kubernetes object state.' },
      { term: 'Scheduler', definition: 'The control plane component that assigns unscheduled Pods to nodes.' },
      { term: 'kubelet', definition: 'The node agent that ensures Pods scheduled to the node are running as specified.' },
      { term: 'Controller', definition: 'A reconciliation loop that watches resources and drives actual state toward desired state.' },
    ],
  },
  {
    ...
    terms: [
      { term: 'Pod', definition: 'The smallest deployable Kubernetes unit, containing one or more tightly coupled containers.' },
      { term: 'Deployment', definition: 'A controller that manages ReplicaSets and rolling updates for stateless workloads.' },
      { term: 'StatefulSet', definition: 'A controller for workloads that need stable identity and ordered lifecycle behavior.' },
      ...
      { term: 'CronJob', definition: 'A scheduled controller that creates Jobs on a recurring timetable.' },
    ],
  },
  {
    ...
    terms: [
      { term: 'Service', definition: 'A stable network identity that routes traffic to a selected set of Pods.' },
      { term: 'Ingress', definition: 'An HTTP routing abstraction for external access into cluster Services.' },
      { term: 'ConfigMap', definition: 'A resource for storing non-sensitive configuration data for workloads.' },
      { term: 'Secret', definition: 'A resource for storing sensitive values such as tokens, passwords, or keys.' },
      { term: 'Readiness probe', definition: 'A health check that determines whether a Pod should receive traffic.' },
      { term: 'Liveness probe', definition: 'A health check that determines whether a container should be restarted.' },
    ],
  },
]
...
const kubernetesHelpStyles = `
.kubernetes-help98-page{min-height:100dvh;background:#c0c0c0;color:#000;font-family:"MS Sans Serif",Tahoma,"Segoe UI",sans-serif;}
.kubernetes-help98-window{width:100%;min-height:100dvh;display:flex;flex-direction:column;background:#c0c0c0;border-top:2px solid #fff;border-left:2px solid #fff;border-right:2px solid #404040;border-bottom:2px solid #404040;box-sizing:border-box;}
.kubernetes-help98-titlebar{position:relative;display:flex;align-items:center;min-height:24px;padding:2px 4px;background:linear-gradient(90deg,#000080 0%,#1084d0 100%);color:#fff;font-size:13px;font-weight:700;}
.kubernetes-help98-title{position:absolute;left:50%;transform:translateX(-50%);font-size:14px;white-space:nowrap;}
.kubernetes-help98-controls{display:flex;gap:2px;margin-left:auto;}
.kubernetes-help98-control{width:18px;height:16px;display:inline-flex;align-items:center;justify-content:center;border-top:1px solid #fff;border-left:1px solid #fff;border-right:1px solid #404040;border-bottom:1px solid #404040;background:#c0c0c0;color:#000;font:inherit;font-size:11px;line-height:1;text-decoration:none;cursor:pointer;}
.kubernetes-help98-tabs{display:flex;flex-wrap:wrap;gap:1px;padding:6px 8px 0;background:#c0c0c0;}
.kubernetes-help98-tab{border-top:1px solid #fff;border-left:1px solid #fff;border-right:1px solid #404040;border-bottom:none;background:#b6b6b6;padding:5px 10px 4px;color:#000;font:inherit;font-size:12px;cursor:pointer;}
.kubernetes-help98-tab.active{position:relative;top:1px;background:#fff;}
.kubernetes-help98-main{display:grid;grid-template-columns:240px minmax(0,1fr);flex:1;min-height:0;border-top:1px solid #404040;background:#fff;}
.kubernetes-help98-toc{overflow:auto;padding:12px;background:#f2f2f2;border-right:1px solid #808080;}
.kubernetes-help98-toc-title{margin:0 0 10px;font-size:12px;font-weight:700;}
.kubernetes-help98-toc-list{margin:0;padding:0;list-style:none;}
.kubernetes-help98-toc-list li{margin:0 0 8px;}
.kubernetes-help98-toc-list a{color:#000;font-size:12px;text-decoration:none;}
.kubernetes-help98-content{overflow:auto;padding:14px 20px 24px;}
.kubernetes-help98-doc-title{margin:0 0 12px;font-size:20px;font-weight:700;}
.kubernetes-help98-section{margin:0 0 20px;}
.kubernetes-help98-heading{margin:0 0 8px;font-size:16px;font-weight:700;}
.kubernetes-help98-content p,.kubernetes-help98-content li,.kubernetes-help98-content dd,.kubernetes-help98-content dt{font-size:12px;line-height:1.5;}
.kubernetes-help98-content p,.kubernetes-help98-content dd{margin:0 0 10px;}
.kubernetes-help98-content ul{margin:0 0 10px 18px;padding:0;}
.kubernetes-help98-divider{margin:14px 0;border:0;border-top:1px solid #d0d0d0;}
.kubernetes-help98-codebox{margin:8px 0 10px;padding:8px;background:#f4f4f4;border-top:2px solid #808080;border-left:2px solid #808080;border-right:2px solid #fff;border-bottom:2px solid #fff;}
.kubernetes-help98-codebox code{display:block;white-space:pre;font-family:"Courier New",Courier,monospace;font-size:12px;line-height:1.45;}
.kubernetes-help98-glossary{margin:0;}
.kubernetes-help98-glossary dt{margin:0 0 2px;font-weight:700;}
@media (max-width:900px){.kubernetes-help98-main{grid-template-columns:1fr;}.kubernetes-help98-toc{border-right:none;border-bottom:1px solid #808080;}.kubernetes-help98-content{padding:14px 14px 20px;}}
`
...
export default function KubernetesPage(): JSX.Element {
  ...
  return (
    <div className="kubernetes-help98-page">
      <style>{kubernetesHelpStyles}</style>
      <div className="kubernetes-help98-window" role="presentation">
        <header className="kubernetes-help98-titlebar">
          <span className="kubernetes-help98-title">{PAGE_TITLE}</span>
          <div className="kubernetes-help98-controls">
            <button className="kubernetes-help98-control" type="button" aria-label="Minimize" onClick={handleMinimize}>
              _
            </button>
            <Link to="/algoViz" className="kubernetes-help98-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="kubernetes-help98-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`kubernetes-help98-tab ${activeTab === tab.id ? 'active' : ''}`}
        ...
        <div className="kubernetes-help98-main">
          <aside className="kubernetes-help98-toc" aria-label="Table of contents">
            <h2 className="kubernetes-help98-toc-title">Contents</h2>
            <ul className="kubernetes-help98-toc-list">
          ...
          <main className="kubernetes-help98-content">
            <h1 className="kubernetes-help98-doc-title">{PAGE_TITLE}</h1>
            {introParagraphs.map((paragraph) => (
              <p key={paragraph}>{paragraph}</p>
            ))}
            <hr className="kubernetes-help98-divider" />
