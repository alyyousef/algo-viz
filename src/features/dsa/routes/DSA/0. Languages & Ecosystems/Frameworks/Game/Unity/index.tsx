...
const glossarySections: GlossarySection[] = [
  {
    ...
    terms: [
      ...
      },
      {
        term: 'GameObject',
        definition:
          'A Unity scene object that acts as a container for components.',
      },
      {
        term: 'Component',
        definition:
          'A modular unit of behavior or capability attached to a GameObject.',
      },
      {
        term: 'MonoBehaviour',
        definition:
      ...
      },
      {
        term: 'Transform',
        definition:
          'The core component that gives every GameObject position, rotation, and scale.',
      },
    ],
  },
  {
    ...
    terms: [
      ...
      },
      {
        term: 'Prefab',
        definition:
          'A reusable GameObject template that can be instantiated and edited centrally.',
      },
      {
...
.unity-help-content p,
.unity-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
function renderContentSection(section: ContentSection, isLast: boolean): JSX.Element {
  return (
    <section key={section.id} id={section.id} className="unity-help-section">
      <h2 className="unity-help-heading">{section.title}</h2>
      ...
      {isLast ? null : <hr className="unity-help-divider" />}
    </section>
  )
}
...
function renderExampleSection(section: ExampleSection, isLast: boolean): JSX.Element {
  return (
    <section key={section.id} id={section.id} className="unity-help-section">
      ...
      <div className="unity-help-codebox">
...
export default function UnityPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Unity (${activeTabLabel})`
  ...
  return (
    <div className="unity-help-page">
      <style>{pageStyles}</style>
      <div className="unity-help-window" role="presentation">
        <header className="unity-help-titlebar">
          <span className="unity-help-titletext">Unity</span>
          <div className="unity-help-controls">
            <button className="unity-help-control" type="button" aria-label="Minimize" onClick={handleMinimize}>
              _
            </button>
            <Link to="/algoViz" className="unity-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="unity-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`unity-help-tab ${activeTab === tab.id ? 'unity-help-tab-active' : ''}`}
        ...
        <div className="unity-help-main">
          <aside className="unity-help-toc" aria-label="Table of contents">
            <h2 className="unity-help-toc-title">Contents</h2>
            <ul className="unity-help-toc-list">
              {sectionLinks[activeTab].map((section) => (
                <li key={section.id} className="unity-help-toc-item">
                  <a href={`#${section.id}`} className="unity-help-toc-link">
                    {section.label}
                  </a>
                </li>
              ))}
          ...
          <main className="unity-help-content">
            <h1 className="unity-help-doc-title">Unity</h1>
