...
const glossaryTerms: readonly GlossaryTerm[] = [
  {
    term: 'Angular',
    definition: 'A full front-end framework for building web applications with components, templates, routing, forms, DI, and official tooling.',
  },
  {
    term: 'Standalone component',
    definition: 'A modern Angular component that can be used without being declared inside an NgModule.',
  },
  {
  ...
  },
  {
    ...
    definition: 'Angular\'s system for providing and consuming shared services or resources through an injector.',
  },
  {
    term: 'Provider',
    definition: 'An Angular registration that tells the injector how to create or supply a dependency.',
  },
  {
    term: 'Signal',
    definition: 'A reactive Angular value that can be read, updated, and used for derived computations.',
  },
  {
    term: 'computed',
  ...
  {
    term: 'RxJS',
    definition: 'A reactive programming library heavily used in Angular for observable streams, async flows, and many existing application patterns.',
  },
  {
    term: 'Reactive forms',
    definition: 'Angular\'s form model centered on explicit form controls, validation, and programmatic form state management.',
  },
  {
    term: 'Template-driven forms',
    definition: 'Angular forms built primarily through template directives and bindings rather than explicit form model classes.',
  },
  {
    term: 'Directive',
    definition: 'An Angular feature that extends template behavior or DOM behavior without always requiring a new component.',
  },
  {
    term: 'Pipe',
  ...
  {
    term: 'Router',
    definition: 'Angular\'s official navigation system for routes, lazy loading, guards, and route hierarchy.',
  },
  {
    term: 'Interceptor',
    definition: 'Angular HTTP middleware used to observe or modify requests and responses centrally.',
  },
  {
    term: 'Hydration',
    definition: 'The client-side activation step after server-rendered Angular markup is delivered to the browser.',
  },
  {
    term: 'NgModule',
    definition: 'An older but still relevant Angular organizational construct that grouped declarations, imports, and providers before standalone APIs became central.',
  },
] as const

const helpStyles = `
.angular-help-page { min-height: 100dvh; background: #c0c0c0; padding: 0; color: #000; font-family: "MS Sans Serif", Tahoma, "Segoe UI", sans-serif; }
.angular-help-window { width: 100%; min-height: 100dvh; display: flex; flex-direction: column; box-sizing: border-box; background: #c0c0c0; border-top: 2px solid #fff; border-left: 2px solid #fff; border-right: 2px solid #404040; border-bottom: 2px solid #404040; }
.angular-help-titlebar { position: relative; display: flex; align-items: center; padding: 2px 4px; background: linear-gradient(90deg, #000080 0%, #1084d0 100%); color: #fff; font-size: 13px; font-weight: 700; }
.angular-help-titletext { position: absolute; left: 50%; transform: translateX(-50%); font-size: 16px; white-space: nowrap; }
.angular-help-controls { display: flex; gap: 2px; margin-left: auto; }
.angular-help-control { width: 18px; height: 16px; display: inline-flex; align-items: center; justify-content: center; border-top: 1px solid #fff; border-left: 1px solid #fff; border-right: 1px solid #404040; border-bottom: 1px solid #404040; background: #c0c0c0; color: #000; font-size: 11px; line-height: 1; text-decoration: none; }
.angular-help-tabs { display: flex; gap: 1px; padding: 6px 8px 0; background: #c0c0c0; }
.angular-help-tab { border-top: 1px solid #fff; border-left: 1px solid #fff; border-right: 1px solid #404040; border-bottom: none; background: #b6b6b6; padding: 5px 10px 4px; font-size: 12px; cursor: pointer; }
.angular-help-tab.is-active { position: relative; top: 1px; background: #fff; }
.angular-help-main { flex: 1; min-height: 0; display: grid; grid-template-columns: 240px 1fr; border-top: 1px solid #404040; background: #fff; }
.angular-help-toc { overflow: auto; padding: 12px; background: #f2f2f2; border-right: 1px solid #808080; }
.angular-help-toc-title { margin: 0 0 10px; font-size: 12px; font-weight: 700; }
.angular-help-toc-list { margin: 0; padding: 0; list-style: none; }
.angular-help-toc-list li { margin: 0 0 8px; }
.angular-help-toc-list a { color: #000; font-size: 12px; text-decoration: none; }
.angular-help-content { overflow: auto; padding: 14px 20px 20px; }
.angular-help-doc-title { margin: 0 0 12px; font-size: 20px; font-weight: 700; }
.angular-help-doc-subtitle { margin: 0 0 12px; font-size: 12px; }
.angular-help-section { margin: 0 0 20px; scroll-margin-top: 12px; }
.angular-help-heading { margin: 0 0 8px; font-size: 16px; font-weight: 700; }
.angular-help-subheading { margin: 0 0 6px; font-size: 13px; font-weight: 700; }
.angular-help-content p, .angular-help-content li { font-size: 12px; line-height: 1.5; }
.angular-help-content p { margin: 0 0 10px; }
.angular-help-content ul { margin: 0 0 10px 20px; padding: 0; }
.angular-help-divider { margin: 14px 0; border: 0; border-top: 1px solid #d0d0d0; }
.angular-help-codebox { margin: 6px 0 10px; padding: 8px; background: #f4f4f4; border-top: 2px solid #808080; border-left: 2px solid #808080; border-right: 2px solid #fff; border-bottom: 2px solid #fff; }
.angular-help-codebox code { display: block; white-space: pre-wrap; font-family: "Courier New", Courier, monospace; font-size: 12px; }
@media (max-width: 900px) { .angular-help-main { grid-template-columns: 1fr; } .angular-help-toc { border-right: none; border-bottom: 1px solid #808080; } .angular-help-titletext { position: static; transform: none; margin: 0 auto 0 0; padding-left: 4px; white-space: normal; } }
`
...
export default function AngularPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Angular (${activeTabLabel})`
  ...
  return (
    <div className="angular-help-page">
      <style>{helpStyles}</style>
      <div className="angular-help-window" role="presentation">
        <header className="angular-help-titlebar">
          <span className="angular-help-titletext">Angular</span>
          <div className="angular-help-controls">
            <button className="angular-help-control" type="button" aria-label="Minimize" onClick={handleMinimize}>
              _
            </button>
            <Link to="/algoViz" className="angular-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="angular-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`angular-help-tab ${activeTab === tab.id ? 'is-active' : ''}`}
        ...
        <div className="angular-help-main">
          <aside className="angular-help-toc" aria-label="Table of contents">
            <h2 className="angular-help-toc-title">Contents</h2>
            <ul className="angular-help-toc-list">
          ...
          <main className="angular-help-content">
            <h1 className="angular-help-doc-title">Angular</h1>
            <p className="angular-help-doc-subtitle">
              Manual-style reference covering overview, standalone APIs, templates, signals, dependency injection,
              routing, forms, SSR story, tradeoffs, and practical examples.
            </p>

            {activeTab === 'big-picture' &&
              bigPictureSections.map((section, index) => (
                <Fragment key={section.id}>
                  <section id={section.id} className="angular-help-section">
                    <h2 className="angular-help-heading">{section.title}</h2>
                    {section.paragraphs.map((paragraph) => (
                      <p key={paragraph}>{paragraph}</p>
                    ))}
                  </section>
                  {index < bigPictureSections.length - 1 && <hr className="angular-help-divider" />}
                </Fragment>
              ))}

            {activeTab === 'core-concepts' &&
              coreConceptSectionsMerged.map((section) => (
                <section key={section.id} id={section.id} className="angular-help-section">
                  <h2 className="angular-help-heading">{section.title}</h2>
                  {section.paragraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
            ...
            {activeTab === 'examples' &&
              examples.map((example) => (
                <section key={example.id} id={example.id} className="angular-help-section">
                  <h2 className="angular-help-heading">{example.title}</h2>
                  <p>{example.description}</p>
                  {example.snippets.map((snippet) => (
                    <Fragment key={`${example.id}-${snippet.label}`}>
                      <h3 className="angular-help-subheading">{snippet.label}</h3>
                      <div className="angular-help-codebox">
            ...
            {activeTab === 'glossary' && (
              <section id="glossary-terms" className="angular-help-section">
                <h2 className="angular-help-heading">Glossary</h2>
