...
.backend98-control:focus-visible,
.backend98-tab:focus-visible,
.backend98-toc-link:focus-visible,
.backend98-inline-link:focus-visible {
...
.backend98-toc-link:hover,
.backend98-toc-link:focus-visible,
.backend98-inline-link:hover,
.backend98-inline-link:focus-visible {
...
export default function BackendFrameworksPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Backend Frameworks (${activeTabLabel})`
  ...
  return (
    <div className="backend98-help-page">
      <style>{pageStyles}</style>
      <div className="backend98-window" role="presentation">
        <header className="backend98-titlebar">
          <span className="backend98-title">Backend Frameworks</span>
          <div className="backend98-controls">
            <button
              className="backend98-control"
              type="button"
              aria-label="Minimize"
            ...
            <Link to="/algoViz" className="backend98-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="backend98-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              ...
              className={`backend98-tab ${activeTab === tab.id ? 'backend98-tab-active' : ''}`}
        ...
        <div className="backend98-main">
          <aside className="backend98-toc" aria-label="Table of contents">
            <h2 className="backend98-toc-title">Contents</h2>
            <ul className="backend98-toc-list">
              {sectionLinks[activeTab].map((section) => (
                <li key={section.id} className="backend98-toc-item">
                  <a href={`#${section.id}`} className="backend98-toc-link">
                    {section.label}
                  </a>
                </li>
              ))}
          ...
          <main className="backend98-content">
            <h1 className="backend98-doc-title">Backend Frameworks</h1>
            ...
            {activeTab === 'glossary' ? (
              <section id="backend98-glossary" className="backend98-section">
                <h2 className="backend98-heading">Glossary</h2>
                {glossary.map((item) => (
                  <p key={item.term}>
