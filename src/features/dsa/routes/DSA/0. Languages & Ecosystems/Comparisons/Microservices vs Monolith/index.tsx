...
.micro-mono-help-controls {
  display: flex;
  gap: 2px;
  margin-left: auto;
}

.micro-mono-help-control {
  width: 18px;
  height: 16px;
  padding: 0;
...
.micro-mono-help-tab-active {
  background: #fff;
  position: relative;
  top: 1px;
}

.micro-mono-help-main {
  flex: 1;
  min-height: 0;
  display: grid;
...
.micro-mono-help-toc-title {
  margin: 0 0 10px;
  font-size: 12px;
  font-weight: 700;
}

.micro-mono-help-toc-list {
  margin: 0;
  padding: 0;
  list-style: none;
}

.micro-mono-help-toc-item {
  margin: 0 0 8px;
}

.micro-mono-help-toc-link {
  color: #000;
  text-decoration: none;
  font-size: 12px;
}

.micro-mono-help-content {
  overflow: auto;
  padding: 14px 20px 20px;
}

.micro-mono-help-doc-title {
  margin: 0 0 10px;
  font-size: 20px;
  font-weight: 700;
}

.micro-mono-help-section {
  margin: 0 0 20px;
}

.micro-mono-help-heading {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 700;
}

.micro-mono-help-content p,
.micro-mono-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
.micro-mono-help-content ul {
  margin: 0 0 10px 20px;
  padding: 0;
}

.micro-mono-help-divider {
  margin: 14px 0;
  border: 0;
  border-top: 1px solid #d0d0d0;
}

.micro-mono-help-codebox {
  margin: 6px 0 10px;
  padding: 8px;
  background: #f4f4f4;
...
function renderContentSection(section: ContentSection, isLast: boolean): JSX.Element {
  return (
    <section key={section.id} id={section.id} className="micro-mono-help-section">
      <h2 className="micro-mono-help-heading">{section.title}</h2>
      ...
      {isLast ? null : <hr className="micro-mono-help-divider" />}
    </section>
  )
}
...
function renderExampleSection(section: ExampleSection, isLast: boolean): JSX.Element {
  return (
    <section key={section.id} id={section.id} className="micro-mono-help-section">
      ...
      <div className="micro-mono-help-codebox">
...
export default function MicroservicesVsMonolithPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Microservices vs Monolith (${activeTabLabel})`
  ...
  return (
    <div className="micro-mono-help-page">
      <style>{pageStyles}</style>
      <div className="micro-mono-help-window" role="presentation">
        <header className="micro-mono-help-titlebar">
          <span className="micro-mono-help-titletext">Microservices vs Monolith</span>
          <div className="micro-mono-help-controls">
            <button
              className="micro-mono-help-control"
              type="button"
              aria-label="Minimize"
            ...
            <Link to="/algoViz" className="micro-mono-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="micro-mono-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`micro-mono-help-tab ${activeTab === tab.id ? 'micro-mono-help-tab-active' : ''}`}
        ...
        <div className="micro-mono-help-main">
          <aside className="micro-mono-help-toc" aria-label="Table of contents">
            <h2 className="micro-mono-help-toc-title">Contents</h2>
            <ul className="micro-mono-help-toc-list">
              {sectionLinks[activeTab].map((section) => (
                <li key={section.id} className="micro-mono-help-toc-item">
                  <a href={`#${section.id}`} className="micro-mono-help-toc-link">
                    {section.label}
                  </a>
                </li>
              ))}
          ...
          <main className="micro-mono-help-content">
            <h1 className="micro-mono-help-doc-title">Microservices vs Monolith</h1>
