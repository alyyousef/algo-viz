...
const glossaryTerms: readonly GlossaryTerm[] = [
  { term: 'ACID', definition: 'A shorthand for transactional guarantees around atomicity, consistency, isolation, and durability.' },
  { term: 'MVCC', definition: 'Multiversion concurrency control, a technique for handling concurrent reads and writes without simple global blocking.' },
  { term: 'InnoDB', definition: 'The MySQL storage engine most commonly used for transactional workloads in modern deployments.' },
  { term: 'JSONB', definition: 'PostgreSQL binary JSON storage optimized for querying and indexing.' },
  { term: 'GIN Index', definition: 'A PostgreSQL index type often used for arrays, full-text search, and JSONB-heavy workloads.' },
  { term: 'GiST Index', definition: 'A PostgreSQL index family useful for certain advanced search and geospatial access patterns.' },
  { term: 'BRIN Index', definition: 'A PostgreSQL index type designed for very large tables where data has useful natural ordering.' },
  { term: 'Read Replica', definition: 'A secondary database instance used primarily for read scaling or recovery scenarios.' },
  { term: 'Logical Replication', definition: 'Replication at the data-change level rather than only block or physical server state level.' },
  { term: 'OLTP', definition: 'Online transaction processing workloads characterized by many short reads and writes.' },
  { term: 'Extension', definition: 'A PostgreSQL capability for adding database-native functionality beyond the core engine.' },
  { term: 'Full-Text Search', definition: 'Database-supported text indexing and search functionality beyond simple substring matching.' },
] as const
...
.pg-mysql-help-controls {
  display: flex;
  gap: 2px;
  margin-left: auto;
}

.pg-mysql-help-control {
  width: 18px;
  height: 16px;
  display: inline-flex;
  align-items: center;
...
.pg-mysql-help-toc-title {
  margin: 0 0 10px;
  font-size: 12px;
  font-weight: 700;
}

.pg-mysql-help-toc-list {
  margin: 0;
  padding: 0;
  list-style: none;
}

.pg-mysql-help-toc-list li {
  margin: 0 0 8px;
}

.pg-mysql-help-toc-list a {
  color: #000000;
  font-size: 12px;
  text-decoration: none;
}

.pg-mysql-help-content {
  overflow: auto;
  padding: 14px 20px 20px;
}

.pg-mysql-help-doc-title {
  margin: 0 0 12px;
  font-size: 20px;
  font-weight: 700;
}

.pg-mysql-help-doc-subtitle {
  margin: 0 0 12px;
  font-size: 12px;
}

.pg-mysql-help-section {
  margin: 0 0 20px;
  scroll-margin-top: 12px;
}

.pg-mysql-help-heading {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 700;
}

.pg-mysql-help-subheading {
  margin: 0 0 6px;
  font-size: 13px;
  font-weight: 700;
}

.pg-mysql-help-content p,
.pg-mysql-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
.pg-mysql-help-content ul {
  margin: 0 0 10px 20px;
  padding: 0;
}

.pg-mysql-help-divider {
  margin: 14px 0;
  border: 0;
  border-top: 1px solid #d0d0d0;
}

.pg-mysql-help-codebox {
  margin: 6px 0 10px;
  padding: 8px;
  background: #f4f4f4;
...
export default function PostgreSqlVsMySqlPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `PostgreSQL vs MySQL (${activeTabLabel})`
  ...
  return (
    <div className="pg-mysql-help-page">
      <style>{helpStyles}</style>
      <div className="pg-mysql-help-window" role="presentation">
        <header className="pg-mysql-help-titlebar">
          <span className="pg-mysql-help-titletext">PostgreSQL vs MySQL</span>
          <div className="pg-mysql-help-controls">
            <button className="pg-mysql-help-control" type="button" aria-label="Minimize" onClick={handleMinimize}>
              _
            </button>
            <Link to="/algoViz" className="pg-mysql-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="pg-mysql-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`pg-mysql-help-tab ${activeTab === tab.id ? 'is-active' : ''}`}
        ...
        <div className="pg-mysql-help-main">
          <aside className="pg-mysql-help-toc" aria-label="Table of contents">
            <h2 className="pg-mysql-help-toc-title">Contents</h2>
            <ul className="pg-mysql-help-toc-list">
          ...
          <main className="pg-mysql-help-content">
            <h1 className="pg-mysql-help-doc-title">PostgreSQL vs MySQL</h1>
            <p className="pg-mysql-help-doc-subtitle">
              Manual-style comparison of architecture, SQL capability, operational fit, and long-term tradeoffs.
            </p>

            {activeTab === 'big-picture' &&
              bigPictureSections.map((section, index) => (
                <Fragment key={section.id}>
                  <section id={section.id} className="pg-mysql-help-section">
                    <h2 className="pg-mysql-help-heading">{section.title}</h2>
                    {section.paragraphs.map((paragraph) => (
                      <p key={paragraph}>{paragraph}</p>
                    ))}
                  </section>
                  {index < bigPictureSections.length - 1 && <hr className="pg-mysql-help-divider" />}
                </Fragment>
              ))}

            {activeTab === 'core-concepts' &&
              coreConceptSections.map((section) => (
                <section key={section.id} id={section.id} className="pg-mysql-help-section">
                  <h2 className="pg-mysql-help-heading">{section.title}</h2>
                  {section.paragraphs.map((paragraph) => (
                    <p key={paragraph}>{paragraph}</p>
                  ))}
            ...
            {activeTab === 'examples' &&
              examples.map((example) => (
                <section key={example.id} id={example.id} className="pg-mysql-help-section">
                  <h2 className="pg-mysql-help-heading">{example.title}</h2>
                  <p>{example.description}</p>
                  {example.snippets.map((snippet) => (
                    <Fragment key={`${example.id}-${snippet.label}`}>
                      <h3 className="pg-mysql-help-subheading">{snippet.label}</h3>
                      <div className="pg-mysql-help-codebox">
            ...
            {activeTab === 'glossary' && (
              <section id="glossary-terms" className="pg-mysql-help-section">
                <h2 className="pg-mysql-help-heading">Glossary</h2>
