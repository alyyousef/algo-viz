...
const pageSubtitle = 'Comparing a container-native serverless platform with a function-native serverless platform.'
...
const examples: ExampleItem[] = [
  {
    ...
    summary: 'Both platforms can return hello-world JSON, but one expects a server and the other expects a handler.',
...
def hello():
  ...
  return {
    ...
    explanation: 'Cloud Run expects a normal web app that listens for HTTP traffic. Lambda expects a function entry point that returns a response object under Lambda invocation semantics.',
  },
  {
    id: 'ex-container',
...
COPY . .
CMD ["index.handler"]`,
    explanation: 'On Cloud Run the image is the application host boundary. On Lambda the image is a package format that still lands inside Lambda function rules.',
  },
  {
    id: 'ex-background',
...
def process_job():
  ...
  for (const record of event.Records) {
    ...
    explanation: 'Cloud Run background work often still looks like sending work to a service or running a job. Lambda background work often looks like reacting directly to a provider-native event batch.',
  },
  {
    id: 'ex-concurrency',
...
export const handler = async (event) => {
  return await loadItem(event.pathParameters.id)
    ...
    explanation: 'Cloud Run can multiplex requests within an instance like an ordinary service. Lambda usually scales by creating more concurrent function execution environments.',
  },
]
...
const glossaryTerms: GlossaryItem[] = [
  {
    ...
    definition: 'The Cloud Run rules that define how a container must start, listen, and behave on the platform.',
  },
  {
  ...
  {
    term: 'Cold start',
    definition: 'Extra startup latency when a new execution environment or container instance must be initialized.',
  },
  {
    term: 'Scale to zero',
  ...
  },
  {
    ...
    definition: 'A design style where work is triggered by events from services, queues, streams, schedules, or storage changes.',
  },
  {
    term: 'Serverless container',
    definition: 'A containerized workload deployed to a platform that manages scaling and infrastructure automatically.',
  },
  {
    term: 'Function-native platform',
    definition: 'A platform whose primary abstraction is the function invocation rather than the service process.',
  },
  {
    term: 'Provisioned concurrency',
    definition: 'An AWS Lambda feature for keeping execution environments initialized to reduce startup latency.',
  },
  {
    term: 'Cloud Run Job',
    definition: 'A Cloud Run workload type for containerized tasks that run to completion instead of serving requests continuously.',
  },
]
...
.cloudrun-lambda-help-controls {
  display: flex;
  gap: 2px;
  margin-left: auto;
}

.cloudrun-lambda-help-control {
  width: 18px;
  height: 16px;
...
.cloudrun-lambda-help-tab.active {
  position: relative;
  top: 1px;
  background: #fff;
}

.cloudrun-lambda-help-main {
  flex: 1;
  min-height: 0;
  display: grid;
...
.cloudrun-lambda-help-toc-title {
  margin: 0 0 10px;
  font-size: 12px;
  font-weight: 700;
}

.cloudrun-lambda-help-toc-list {
  list-style: none;
  margin: 0;
  padding: 0;
}

.cloudrun-lambda-help-toc-list li {
  margin: 0 0 8px;
}

.cloudrun-lambda-help-toc-list a {
  color: #000;
  text-decoration: none;
  font-size: 12px;
}

.cloudrun-lambda-help-content {
  overflow: auto;
  padding: 14px 20px 20px;
}

.cloudrun-lambda-help-doc-title {
  margin: 0 0 6px;
  font-size: 20px;
  font-weight: 700;
}

.cloudrun-lambda-help-doc-subtitle {
  margin: 0 0 12px;
  font-size: 13px;
  font-weight: 700;
}

.cloudrun-lambda-help-section {
  margin: 0 0 20px;
}

.cloudrun-lambda-help-heading {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 700;
}

.cloudrun-lambda-help-subheading {
  margin: 0 0 6px;
  font-size: 13px;
  font-weight: 700;
}

.cloudrun-lambda-help-content p,
.cloudrun-lambda-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
.cloudrun-lambda-help-content ul {
  margin: 0 0 10px 20px;
  padding: 0;
}

.cloudrun-lambda-help-divider {
  border: 0;
  border-top: 1px solid #d0d0d0;
  margin: 14px 0;
}

.cloudrun-lambda-help-codebox {
  margin: 6px 0 10px;
  padding: 8px;
...
.cloudrun-lambda-help-codebox code {
  display: block;
...
export default function CloudRunVsAwsLambdaPage(): JSX.Element {
  ...
  return (
    <div className="cloudrun-lambda-help-page">
      <style>{pageStyles}</style>
      <div className="cloudrun-lambda-help-window" role="presentation">
        <header className="cloudrun-lambda-help-titlebar">
          <span className="cloudrun-lambda-help-title">{pageTitle}</span>
          <div className="cloudrun-lambda-help-controls">
            <button className="cloudrun-lambda-help-control" type="button" aria-label="Minimize" onClick={handleMinimize}>
              _
            </button>
            <Link to="/algoViz" className="cloudrun-lambda-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="cloudrun-lambda-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`cloudrun-lambda-help-tab ${activeTab === tab.id ? 'active' : ''}`}
        ...
        <div className="cloudrun-lambda-help-main">
          <aside className="cloudrun-lambda-help-toc" aria-label="Table of contents">
            <h2 className="cloudrun-lambda-help-toc-title">Contents</h2>
            <ul className="cloudrun-lambda-help-toc-list">
          ...
          <main className="cloudrun-lambda-help-content">
            <h1 className="cloudrun-lambda-help-doc-title">{pageTitle}</h1>
            ...
            <p>
              ...
              The point is not to memorize product marketing. The point is to understand the deeper tradeoffs:
              execution model, packaging boundary, request handling, event integration, concurrency behavior,
              startup profile, portability, operational fit, and the type of team each platform usually serves best.
            </p>

            {activeTab === 'big-picture' && (
              <>
                {bigPictureSections.map((section, index) => (
                  <section key={section.id} id={section.id} className="cloudrun-lambda-help-section">
                    <h2 className="cloudrun-lambda-help-heading">{section.title}</h2>
                    ...
                    {index < bigPictureSections.length - 1 && <hr className="cloudrun-lambda-help-divider" />}
                  </section>
                ))}
              </>
            )}
            ...
            {activeTab === 'core-concepts' && (
              <>
                <section id="core-mental" className="cloudrun-lambda-help-section">
                  <h2 className="cloudrun-lambda-help-heading">Mental Models</h2>
            ...
            {activeTab === 'examples' && (
              <>
                {examples.map((example) => (
                  <section key={example.id} id={example.id} className="cloudrun-lambda-help-section">
                    <h2 className="cloudrun-lambda-help-heading">{example.title}</h2>
                    <p>{example.summary}</p>
                    <h3 className="cloudrun-lambda-help-subheading">Cloud Run</h3>
                    <div className="cloudrun-lambda-help-codebox">
                      <code>{example.cloudRunCode.trim()}</code>
                    </div>
                    <h3 className="cloudrun-lambda-help-subheading">AWS Lambda</h3>
                    <div className="cloudrun-lambda-help-codebox">
            ...
            {activeTab === 'glossary' && (
              <section id="glossary-terms" className="cloudrun-lambda-help-section">
                <h2 className="cloudrun-lambda-help-heading">Glossary</h2>
