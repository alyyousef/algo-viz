...
const mentalModels = [
  ...
  },
  {
    ...
    detail: 'Small functions compose into larger behavior without deep class hierarchies.',
  },
]
...
const applications = [
  ...
  },
  {
    ...
    detail: 'Functional pipelines enable reproducible transformations and clean data lineage.',
  },
  {
    context: 'Streaming systems',
    detail: 'Functional streams model continuous data with composable transformations.',
  },
]
...
firstTenSquares = take 10 (map (^2) naturals)`,
  ...
  switch (action.type) {
    ...
    explanation: 'Reducers make state transitions explicit and easy to test without side effects.',
  },
  {
    title: 'Function composition',
    code: `const pipe = (...fns) => (x) => fns.reduce((v, f) => f(v), x)
const normalize = pipe(trim, toLower, removePunctuation)`,
    explanation: 'Composition builds pipelines from small reusable transformations.',
  },
]
...
const advancedInsights = [
  ...
  },
  {
    ...
    detail: 'Concepts like functors and monoids formalize composition and reusable abstractions.',
  },
]
...
const fpTooling = [
  ...
  {
    title: 'Tooling',
    detail: 'REPLs, property-based testing, and type-driven tooling accelerate feedback loops.',
  },
  {
    title: 'Runtime support',
    detail: 'Runtimes with good profilers and GC insights are critical for performance tuning.',
  },
]
...
const debuggingWorkflow = [
  {
    ...
    detail: 'Measure heap growth and GC pauses to detect space leaks or hot paths.',
  },
  {
    title: 'Check strictness',
    detail: 'Validate evaluation strategy to ensure thunks are not piling up.',
  },
  {
    title: 'Property-based tests',
    detail: 'Generators explore edge cases that example tests often miss.',
  },
  {
    title: 'Trace effects',
    detail: 'Log and observe effect boundaries to ensure IO and state changes are isolated.',
  },
]
...
const productionChecklist = [
  {
    title: 'Performance',
    detail: 'Profile pipelines, limit allocations, and apply strictness where needed.',
  },
  {
    title: 'Correctness',
    detail: 'Use types and property tests to encode domain invariants.',
  },
  {
    title: 'Reliability',
    detail: 'Isolate side effects and use pure cores with effectful shells.',
  },
  {
    title: 'Team alignment',
    detail: 'Document FP conventions to avoid abstraction sprawl.',
  },
]
...
export default function FunctionalProgrammingPage(): JSX.Element {
  const { activeTab, setActiveTab, handleMinimize } = useTopicTabs({
    tabs,
    pageTitle: 'Functional Programming',
  ...
  return (
    ...
    >
      ...
      <p>
        Functional programming treats computation as the evaluation of expressions, not the mutation
        of state. By emphasizing pure functions, immutability, and composition, it makes programs
        easier to reason about, test, and parallelize, while demanding care with performance and
        effect management.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview">
            <h2>Overview</h2>
            <p>
              The paradigm reduces moving parts: state changes are explicit, and functions are the
              primary building blocks. This clarity aids correctness and concurrency but shifts
              responsibility to manage performance overheads from allocation, laziness, and runtime
          ...
          <section id="bp-complexity">
            ...
            <p>
              Start with clean, pure designs, then profile. Optimize by fusing pipelines,
              controlling strictness, and using specialized data structures. If mutation is needed
              for speed, isolate it behind pure interfaces.
            </p>
          </section>
      ...
      {activeTab === 'glossary' && (
        <>
          <section id="glossary-terms">
            ...
            <p>
              <strong>Pure function:</strong> A function with no side effects that returns the same
              output for the same input.
            </p>
            <p>
              <strong>Referential transparency:</strong> Replacing an expression with its value does
              not change program behavior.
            </p>
            <p>
              <strong>Immutability:</strong> Values are not modified in place; new values are
              derived from existing values.
            </p>
            <p>
              <strong>Higher-order function:</strong> A function that takes other functions as
              arguments or returns functions.
            </p>
            <p>
              <strong>Algebraic data type (ADT):</strong> A type built from product and sum
              combinations to model domain shapes safely.
            </p>
            <p>
              <strong>Pattern matching:</strong> Structural branching over ADT variants with
              compiler-checked exhaustiveness.
            </p>
            <p>
              <strong>Type inference:</strong> Compiler deduction of types, often associated with
              Hindley-Milner systems.
            </p>
            <p>
              <strong>Laziness (non-strict evaluation):</strong> Deferring evaluation until values
              are demanded.
            </p>
            <p>
              <strong>Persistent data structure:</strong> An immutable structure using structural
              sharing for efficient updates.
            </p>
            <p>
              <strong>Monads and effect systems:</strong> Techniques for sequencing and constraining
              side effects while preserving a pure core.
            </p>
          </section>
