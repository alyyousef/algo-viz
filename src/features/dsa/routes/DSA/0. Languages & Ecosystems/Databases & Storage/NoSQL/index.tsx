...
const queryPatterns = [
  ...
  {
    title: 'Range scans',
    detail: 'Sort keys (timestamps, sequence IDs) enable fast range queries within a partition.',
  },
  {
    title: 'Fan-out reads',
    detail:
...
const storageEngines = [
  ...
  },
  {
    ...
    detail: 'Prioritize speed with RAM-first storage; durability depends on snapshots and logs.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Enable flexible querying but add write amplification and storage cost.',
  },
  {
    title: 'Change streams',
    detail: 'Expose updates as ordered logs for downstream processing and materialized views.',
  },
]
const consistencyMechanisms = [
  ...
  {
    title: 'Read repair',
    detail: 'Background reconciliation fixes stale replicas after reads detect divergence.',
  },
  {
    title: 'Hinted handoff',
    detail: 'Temporary writes are queued when a replica is down, then replayed later.',
  },
  {
    title: 'Vector clocks',
    detail: 'Track causality to detect conflicts and enable resolution strategies.',
  },
  {
    title: 'CRDTs',
    detail: 'Conflict-free replicated data types merge automatically for eventual consistency.',
  },
  {
    title: 'Transactional fences',
    detail: 'Lightweight transactions or CAS operations provide conditional updates when needed.',
  },
]
...
const transactionModels = [
  {
    ...
    detail: 'Most NoSQL systems guarantee atomic updates within a single key/document.',
  },
  {
    title: 'Multi-record transactions',
    detail: 'Some engines provide limited multi-document transactions with higher latency.',
  },
  {
    title: 'Conditional updates',
    detail: 'Compare-and-set or version checks prevent lost updates in concurrent writers.',
  },
  {
    title: 'Idempotent writes',
    detail: 'Design APIs so retries do not duplicate effects, especially in eventual consistency.',
  },
]
...
const operationsChecklist = [
  {
    ...
    detail: 'Validate restore procedures; many NoSQL systems require special snapshot flows.',
  },
  {
    title: 'Capacity planning',
    detail: 'Plan for compaction overhead, replication, and index storage growth.',
  },
  {
    title: 'Monitoring and alerts',
    detail: 'Track tail latency, replication lag, compaction queue, and hot partitions.',
  },
  {
    title: 'Schema validation',
    detail: 'Enforce contracts at the app layer or via schema validators to prevent drift.',
  },
  {
    title: 'TTL and data lifecycle',
    detail: 'Use retention policies to bound storage and avoid hot partitions.',
  },
  {
    title: 'Load testing',
    detail: 'Simulate read/write mixes and burst traffic; tune partition keys accordingly.',
  },
]
...
const migrationStrategies = [
  {
    ...
    detail: 'Write to both old and new stores, backfill historical data, then cut over.',
  },
  {
    title: 'Change data capture',
    detail: 'Stream updates from the primary DB into NoSQL for read scaling or analytics.',
  },
  {
    title: 'Read-through cache',
    detail: 'Gradually populate a NoSQL cache with production traffic to reduce risk.',
  },
  {
    title: 'Feature-flagged cutover',
    detail: 'Use flags to switch traffic per cohort, enabling rollback if issues appear.',
  },
]
...
const dataModelingPatterns = [
  ...
  },
  {
    ...
    detail: 'Precompute read-optimized projections to avoid expensive aggregations at query time.',
  },
]
...
export default function NoSqlPage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">NoSQL</h1>
      <p>
        NoSQL databases trade rigid relational schemas for specialized data models that scale
        horizontally and adapt quickly. Instead of one universal model, NoSQL offers families of
        databases optimized for particular access patterns, from key-value lookups to graph
        traversals.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              NoSQL systems prioritize scale-out architectures, flexible schemas, and specialized
              query patterns. They often embrace eventual consistency to maximize availability and
              performance, but many provide knobs for stronger guarantees.
            </p>
          </section>
      ...
      {activeTab === 'core-concepts' && (
        <>
          <section id="core-families" className="bin98-section">
            <h2 className="bin98-heading">How It Works: NoSQL Families</h2>
          ...
          <section id="core-architecture" className="bin98-section">
            <h2 className="bin98-heading">How It Works: Architecture Building Blocks</h2>
          ...
          <section id="core-storage" className="bin98-section">
            <h2 className="bin98-heading">Storage Engines and Indexing</h2>
          ...
          <section id="core-cap" className="bin98-section">
            <h2 className="bin98-heading">Consistency and CAP Tradeoffs</h2>
            ...
            <p>
              CAP is not a feature checklist; it is a tradeoff triangle. During network partitions,
              you choose between consistency and availability. Many NoSQL systems expose tuning
              knobs so each workload can choose the right point on that triangle.
            </p>
          </section>
          <section id="core-mechanisms" className="bin98-section">
            <h2 className="bin98-heading">Consistency Mechanics in the Real World</h2>
          ...
          <section id="core-transactions" className="bin98-section">
            <h2 className="bin98-heading">Transaction and Constraint Models</h2>
          ...
          <section id="core-modeling" className="bin98-section">
            <h2 className="bin98-heading">Data Modeling Patterns</h2>
          ...
          <section id="core-migrations" className="bin98-section">
            <h2 className="bin98-heading">Migration and Integration Strategies</h2>
