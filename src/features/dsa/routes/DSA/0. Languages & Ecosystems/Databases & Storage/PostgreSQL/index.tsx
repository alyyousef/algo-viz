...
const glossarySections: GlossarySection[] = [
  {
    ...
    terms: [
      {
        term: 'Schema',
        definition: 'A namespace inside a PostgreSQL database used to organize tables, views, functions, and other objects.',
      },
      {
        term: 'MVCC',
        definition: 'Multi-version concurrency control, the mechanism PostgreSQL uses so readers and writers can interact with less blocking.',
      },
      {
        term: 'WAL',
        definition: 'Write-ahead log used for durability, crash recovery, replication, and point-in-time restore.',
      },
      {
        term: 'JSONB',
        definition: 'A binary JSON representation in PostgreSQL that supports indexing and query operations.',
      },
      {
        term: 'Role',
      ...
      {
        term: 'Extension',
        definition: 'A package that adds functionality to PostgreSQL, such as new types, operators, or functions.',
      },
    ],
  },
  {
    ...
    terms: [
      ...
      {
        term: 'Analyze',
        definition: 'Statistic collection that helps the PostgreSQL planner estimate row counts and choose plans.',
      },
      {
        term: 'Partition',
      ...
      },
      {
        ...
        definition: 'A PostgreSQL command that runs a query and reports its actual execution plan and timing.',
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition: 'Replication of PostgreSQL storage changes at the WAL level to standby servers.',
      },
      {
        term: 'Logical replication',
        definition: 'Replication of selected data changes in a more object-aware form than raw physical replication.',
      },
      {
        term: 'Point-in-time recovery',
        definition: 'Restoring a PostgreSQL database to a chosen point using backups plus WAL replay.',
      },
      {
        term: 'Connection pool',
        definition: 'A managed pool of reusable database connections used to limit overhead and control load.',
      },
      {
      ...
      },
      {
        ...
        definition: 'The strongest standard PostgreSQL isolation level, designed to prevent transaction anomalies.',
      },
    ],
  },
]
...
export default function PostgreSqlPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `PostgreSQL (${activeTabLabel})`
    ...
    <div className="postgres-help-page">
      ...
      <div className="postgres-help-window" role="presentation">
        <header className="postgres-help-titlebar">
          <span className="postgres-help-titletext">PostgreSQL</span>
        ...
        <div className="postgres-help-main">
          ...
          <main className="postgres-help-content">
            <h1 className="postgres-help-doc-title">PostgreSQL</h1>
            <p className="postgres-help-doc-subtitle">
              Relational database server reference covering transactions, MVCC, WAL, indexing,
              extensions, replication, partitioning, and operational tradeoffs.
            </p>
