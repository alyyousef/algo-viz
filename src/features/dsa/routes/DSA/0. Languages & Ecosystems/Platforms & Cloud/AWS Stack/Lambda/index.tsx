...
const glossaryTerms = [
  ...
  {
    term: 'Execution role',
    definition: 'The IAM role under which the Lambda function code runs and calls AWS APIs.',
  },
]
...
export default function AwsLambdaPage(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <p>
        This page focuses on Lambda as an execution platform rather than as a simple handler
        function. The important topics are cold starts, concurrency, trigger semantics, failure
        behavior, IAM, packaging, and how the function fits into an event-driven system around it.
      </p>
      <p>
      ...
      {activeTab === 'big-picture' && (
        <>
          ...
          <section id="bp-execution" className="bin98-section">
            <h2 className="bin98-heading">Execution Choices</h2>
          ...
          <section id="bp-fit" className="bin98-section">
            <h2 className="bin98-heading">When to Choose Lambda</h2>
            <ul>
              {fitGuide.map((item) => (
                <li key={item.title}>
          ...
          <ul>
            {pageSources.map((source) => (
              <li key={source}>
                <a
                  href={source}
                  className="lambda-help-inline-link"
                  target="_blank"
                  rel="noreferrer"
                >
                  {source}
