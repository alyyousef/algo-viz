...
const pageSubtitle =
  'Managed Kubernetes on Azure for production clusters, platform teams, and cloud-native workloads.'
...
const fitGuide = [
  ...
  },
  {
    ...
    choice:
      'Use multiple clusters, subscriptions, VNets, or landing zones where appropriate instead of forcing everything into one shared cluster.',
  },
  {
    title: 'Need Linux and Windows containers in one managed Kubernetes platform',
    choice:
      'AKS supports mixed operating systems through separate node pools, with the usual Kubernetes scheduling constraints.',
  },
]
...
const glossaryTerms = [
  ...
  },
  {
    term: 'Control plane',
    definition:
      'The managed Kubernetes management components such as the API server, scheduler, and etcd.',
  },
  {
    term: 'System node pool',
    definition:
      'The required AKS node pool mode that hosts critical cluster add-ons and system workloads.',
  },
  {
    term: 'User node pool',
    definition:
      'A node pool intended primarily for application workloads and workload-specific capacity design.',
  },
  {
    term: 'Azure CNI Overlay',
    definition:
      'An AKS networking mode that combines Azure networking integration with overlay-based pod IP allocation.',
  },
  {
    term: 'kubenet',
    definition:
      'An older AKS networking model that Microsoft documents as retiring from support on March 31, 2028.',
  },
  {
    term: 'Workload Identity',
    definition:
      'The AKS identity model that uses OIDC federation between Kubernetes service accounts and Microsoft Entra identities.',
  },
  {
    term: 'Managed identity',
    definition:
      'An Azure identity used by AKS components or workloads to access Azure resources without managing secrets manually.',
  },
  {
    term: 'Cluster Autoscaler',
    definition:
      'The component that adjusts node counts in AKS node pools based on unschedulable pods and utilization signals.',
  },
  {
    term: 'Horizontal Pod Autoscaler',
    definition:
      'A Kubernetes autoscaler that changes pod replica counts based on observed metrics.',
  },
  {
    term: 'CSI',
    definition:
      'Container Storage Interface, the plugin model used in AKS for Azure Disk, Azure Files, and related storage integrations.',
  },
  {
    term: 'Private cluster',
    definition:
      'An AKS cluster whose API server is exposed through private networking rather than only through a public endpoint.',
  },
  {
    term: 'Authorized IP ranges',
    definition:
      'A control that limits access to a public AKS API server to specified public IP address ranges.',
  },
  {
    term: 'Availability zone',
    definition:
      'A physically separate Azure datacenter zone in a region used for higher availability and failure-domain separation.',
  },
  {
    term: 'AKS Automatic',
    definition:
      'A more opinionated AKS mode in which Microsoft manages more cluster and node behavior for you.',
  },
]
const pageSources = [
...
export default function AzureAKSPage(): JSX.Element {
  const { activeTab, setActiveTab, handleMinimize } = useTopicTabs({
    tabs,
    pageTitle: 'Azure A K S Page',
    defaultTab: 'big-picture',
  })

  return (
    <TopicPageShell
      title="Azure A K S Page"
      tabs={tabs}
      activeTab={activeTab}
        ...
        <>
          ...
          <section id="glossary-sources" className="bin98-section">
            ...
            <p>
              ...
              March 12, 2026. AKS features, support policy details, region availability, and
              retirement timelines can change, so production decisions should always be verified
              against the current Azure documentation.
            </p>
            <ul>
