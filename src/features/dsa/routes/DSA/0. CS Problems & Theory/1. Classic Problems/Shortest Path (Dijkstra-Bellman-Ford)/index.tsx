...
const historicalContext = [
  {
    ...
    notes: 'It is optimal due to a monotonicity property of non-negative edge weights.',
  },
  {
    title: '1958: Bellman-Ford',
    details:
      'Introduced a relaxation-based algorithm that tolerates negative edges and detects negative cycles.',
    notes: 'It became a standard tool in network routing and theoretical proofs.',
  },
  {
    title: 'Modern era',
    details:
      'Shortest-path algorithms power web-scale routing, logistics optimization, and graph analytics.',
    notes: 'Research focuses on heuristics, preprocessing, and specialized graph families.',
  },
]
...
const formalDefinitions = [
  {
    ...
    detail: 'dist[v] = min over all s->v paths of sum of edge weights (or INF if unreachable).',
  },
  {
    title: 'Triangle inequality under relaxation',
    detail: 'For any edge (u,v), a valid solution must satisfy dist[v] <= dist[u] + w(u,v).',
  },
  {
  ...
  },
  {
    ...
    detail: 'If any edge can still relax after |V|-1 iterations, a negative cycle is reachable.',
  },
]
...
const complexityNotes = [
  ...
  },
  {
    ...
    detail:
      'Dijkstra with a heap is best on sparse graphs; array-based Dijkstra may be OK on dense graphs.',
  },
  {
...
Shortest A->D: A->C->B->D cost 8`,
    explanation: 'Dijkstra is correct here because all weights are non-negative.',
  },
  {
    title: 'Graph with negative edge',
    code: `Edges: A->B(2), B->C(-4), A->C(5)
Shortest A->C: A->B->C cost -2`,
    explanation: 'Bellman-Ford handles this correctly; Dijkstra may fail.',
  },
  {
    title: 'Negative cycle',
    code: `Cycle: X->Y(1), Y->Z(-3), Z->X(1)
Total cycle weight: -1`,
    explanation: 'Shortest paths are undefined since the cycle can reduce cost indefinitely.',
  },
]
...
repeat V times:
  ...
  for each edge u->v:
    if dist[u] + w(u,v) < dist[v]:
      dist[v] = dist[u] + w(u,v)`,
    explanation: 'The greedy choice is correct only with non-negative weights.',
  },
  {
    title: 'Bellman-Ford',
    code: `dist[s]=0; others=INF
...
one more pass:
  if any edge relaxes -> negative cycle`,
    explanation: 'Repeated relaxation guarantees optimal distances if no negative cycle exists.',
  },
]
...
export default function ShortestPathDijkstraBellmanFordPage(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <p>
        This page presents the shortest path problem in a more academic style: precise definitions,
        correctness conditions, proof sketches, and complexity analysis. The focus is on Dijkstra
        and Bellman-Ford as canonical algorithms with contrasting assumptions and guarantees.
      </p>
      <p>
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-correctness" className="bin98-section">
            <h2 className="bin98-heading">Correctness Sketches</h2>
