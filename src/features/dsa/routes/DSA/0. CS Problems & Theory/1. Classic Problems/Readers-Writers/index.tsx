...
const bigPicture = [
  {
    ...
    notes: 'The goal is to maximize read throughput while preserving correctness for writes.',
  },
  {
    title: 'Why it matters',
    details: 'It models databases, file systems, caches, and shared in-memory data structures.',
    notes: 'Designing the policy determines whether readers or writers can starve.',
  },
  {
    title: 'What it teaches',
    details:
      'How to separate safety (no concurrent read/write conflicts) from fairness and performance.',
    notes: 'It highlights the tradeoff between concurrency and latency.',
  },
]
...
const problemSetup = [
  ...
  },
  {
    title: 'Constraint',
    detail:
      'A writer must wait until no readers are active; readers must wait if a writer is active.',
  },
]
...
const synchronizationPatterns = [
  ...
  },
  {
    ...
    detail:
      'Count readers with a mutex; use a write lock to block writers while readers are active.',
  },
  {
...
Policy: Writer preference`,
    explanation: 'Writers make progress, but readers can experience long delays.',
  },
  {
...
Writers: 10
Policy: Fair FIFO`,
    explanation: 'No starvation, but the overhead of fairness reduces peak throughput.',
  },
]
...
writer():
  ...
  broadcast(canRead)
  unlock(m)`,
    explanation: 'New readers can enter as long as no writer is active, which can starve writers.',
  },
  {
...
export default function ReadersWritersPage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Readers-Writers</h1>
      <p>
        The readers-writers problem models access to shared data where reads are frequent and writes
        must be exclusive. The core challenge is deciding who waits: should readers stream through
        while writers queue, or should writers get priority to prevent starvation? This document
        explains the main policies, correctness invariants, and practical tradeoffs.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          ...
          <section id="bp-claims" className="bin98-section">
            ...
            <p>
              Safety enforces exclusivity for writers. Fairness and throughput depend on which
              policy you choose to schedule readers and writers.
            </p>
          </section>
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-policies" className="bin98-section">
            <h2 className="bin98-heading">Policy Variants</h2>
      ...
      {activeTab === 'examples' && (
        <>
          ...
          <section id="ex-policy" className="bin98-section">
            <h2 className="bin98-heading">Policy Selector</h2>
            <p>
            ...
            <h3 className="bin98-subheading">{activePolicy?.name}</h3>
            <p>{activePolicy?.summary}</p>
          </section>

          <section id="ex-queue" className="bin98-section">
            <h2 className="bin98-heading">Queue Controls</h2>
            <p>
            ...
            <p>
              <strong>Waiting readers:</strong> {waitingReaders}
            </p>
            <p>
              <strong>Waiting writers:</strong> {waitingWriters}
            </p>
            <p>
              <strong>Queue:</strong>{' '}
              {queue.length ? queue.map((item) => item.type).join(' ') : 'Empty'}
            </p>
          </section>

          <section id="ex-scheduler" className="bin98-section">
            <h2 className="bin98-heading">Scheduler Stepper</h2>
            <p>
              Step the scheduler and finish active readers or writers. This is a conceptual model
              for intuition.
            </p>
            <p>
              <strong>Active readers:</strong> {activeReaders}
            </p>
            <p>
