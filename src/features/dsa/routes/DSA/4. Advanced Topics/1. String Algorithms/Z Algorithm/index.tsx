...
export default function ZAlgorithmPage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Z Algorithm</h1>
      <p>
        The Z algorithm computes, for every index, how long the substring matches the prefix. With a
        clever Z-box window, it avoids redundant comparisons and powers fast pattern matching,
        border detection, and repetition analysis.
      </p>
      <p>
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              Z turns a string into an array of prefix match lengths. That single preprocessing step
              provides immediate answers to many classic string tasks, including pattern search and
              border discovery, all in linear time.
            </p>
          ...
          <section id="bp-why" className="bin98-section">
            ...
            <p>
              The Z-box is the key: it stores a verified matching segment to reuse comparisons. Z
              offers linear preprocessing and clean pattern-matching tricks, but needs the full
              string up front.
            </p>
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-construction" className="bin98-section">
            <h2 className="bin98-heading">Z-Box Construction</h2>
            {buildSteps.map((item) => (
              <p key={item.title}>
                <strong>{item.title}:</strong> {item.detail}
              </p>
          ...
          <section id="core-matching" className="bin98-section">
            <h2 className="bin98-heading">Pattern Matching Workflow</h2>
            {matchSteps.map((item) => (
              <p key={item.title}>
                <strong>{item.title}:</strong> {item.detail}
              </p>
      ...
      {activeTab === 'examples' && (
        <>
          ...
          <section id="ex-walkthrough" className="bin98-section">
            ...
            <h3 className="bin98-subheading">Building the Z values</h3>
            <ol>
              {buildSteps.map((item) => (
                <li key={item.title}>
                  <strong>{item.title}:</strong> {item.detail}
            ...
            <h3 className="bin98-subheading">Finding occurrences</h3>
            <ol>
              {matchSteps.map((item) => (
                <li key={item.title}>
                  <strong>{item.title}:</strong> {item.detail}
