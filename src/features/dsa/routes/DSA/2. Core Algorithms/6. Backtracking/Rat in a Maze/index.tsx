...
const historicalMilestones = [
  {
    ...
    detail: 'Maze traversal puzzles inspired early algorithmic thinking about paths and dead ends.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Rat in a Maze became a go-to educational problem for recursion and state undo.',
  },
]
...
const mentalModels = [
  {
    ...
    detail: 'Try a direction, but if you hit a wall or a loop, back up and try another path.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Each move is a branch in a tree of choices. Backtracking prunes dead branches.',
  },
]
...
const coreIdeas = [
  ...
  },
  {
    ...
    detail: 'Move step by step. If a move leads to a dead end, undo and try another direction.',
  },
  {
    title: 'Path marking',
    detail: 'Mark the current cell in a solution matrix. Unmark it when backtracking.',
  },
  {
    title: 'Multiple solutions',
    detail: 'You can stop at the first path or continue to enumerate all possible paths.',
  },
]
...
const dataStructures = [
  {
    title: 'Maze grid',
    detail: 'A 2D array where 1 means open and 0 means blocked. Defines legal positions.',
  },
  {
    title: 'Solution matrix',
    detail: 'Tracks the current path. Useful for printing or returning the path.',
  },
  {
    title: 'Visited set',
    detail: 'If you allow four directions, a visited tracker prevents cycles.',
  },
  {
    title: 'Move list',
    detail:
...
const correctnessNotes = [
  ...
  {
    title: 'Soundness',
    detail: 'Only valid moves to open cells are taken, so any solution returned is valid.',
  },
  {
    title: 'Termination',
    detail: 'The recursion depth is bounded by the number of cells; visited checks prevent cycles.',
  },
]
...
const complexityNotes = [
  ...
  },
  {
    ...
    detail: 'O(n^2) for the grid and solution matrix, plus recursion depth up to n^2.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Listing all solutions can be huge; stopping after the first keeps the search short.',
  },
]
...
const edgeCases = [
  {
    ...
    detail: 'If the start or destination cell is blocked, there is no solution.',
  },
  {
    title: 'Single-cell maze',
    detail: 'A 1x1 grid is trivially solved if the cell is open.',
  },
  {
...
const realWorldUses = [
  {
    ...
    detail: 'Robots in grid worlds must find collision-free paths with obstacles.',
  },
  {
    context: 'Puzzle solving',
    detail: 'Maze-based games and puzzles use similar backtracking to verify solvability.',
  },
  {
    context: 'Routing',
    detail: 'Finding routes in constrained spaces with obstacles mirrors grid traversal.',
  },
  {
    context: 'AI search demos',
    detail: 'Rat in a Maze is a simple demonstration of DFS and backtracking.',
  },
  {
    context: 'Constraint satisfaction',
    detail: 'It models decision sequences with constraints and dead ends.',
  },
]
...
const examples = [
  {
    ...
    unmark (x, y)
    return false`,
    explanation: 'Try a move, recurse, and backtrack if it fails. The first success yields a path.',
  },
  {
    title: 'Right and down only',
    code: `moves = [(0, 1), (1, 0)]
// This version only searches monotonic paths.`,
    explanation: 'Restricting to right/down reduces branching but can miss valid solutions.',
  },
  {
...
for each move:
    ...
    explanation: 'Do not stop at the first success if you want all possible solutions.',
  },
]
...
const variants = [
  {
    ...
    detail: 'Use BFS or Dijkstra instead of backtracking to guarantee shortest paths.',
  },
  {
    title: 'Weighted cells',
    detail: 'Costs on cells turn it into a weighted pathfinding problem.',
  },
  {
    title: 'Multiple destinations',
    detail: 'Stop when reaching any of several goal cells, or enumerate paths to all of them.',
  },
  {
    title: 'Diagonal moves',
    detail: 'Allowing diagonals increases branching and requires updated validity checks.',
  },
]
...
const glossaryTerms = [
  ...
  {
    term: 'Maze grid',
    definition: 'A 2D array where open cells can be visited and blocked cells cannot.',
  },
  {
    term: 'Solution matrix',
    definition: 'A companion matrix that records the cells currently included in the path.',
  },
  {
    term: 'Visited set',
    definition:
...
export default function RatInMazePage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Rat in a Maze</h1>
      <p>
        Rat in a Maze is a classic backtracking problem: move through a grid from start to finish
        while avoiding blocked cells. The algorithm tries steps, records the path, and backtracks
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              The maze is a grid graph with obstacles. Backtracking searches for a path by exploring
              moves, undoing them when they fail. It is a simple but powerful example of depth-first
              search on a constrained space.
            </p>
            {coreIdeas.map((item) => (
              <p key={item.title}>
      ...
      {activeTab === 'core-concepts' && (
        <>
          <section id="core-foundation" className="bin98-section">
            ...
            <p>
              The backtracking approach moves step by step through the maze and undoes work whenever
              a branch cannot reach the goal. That simple pattern turns the puzzle into a
              disciplined search procedure rather than random trial and error.
            </p>
            <p>
              Backtracking explores all reachable paths consistent with the move rules, so it is
      ...
      {activeTab === 'examples' && (
        <>
          ...
          <section id="examples-notes" className="bin98-section">
            ...
            <p>
              The example set covers the core recursive solver, a restricted move list, and a
              version that keeps exploring after reaching the destination so every valid path can be
              listed.
            </p>
            <p>
              These examples reinforce the same underlying rules: valid moves only, explicit path
              marking, and careful undo logic during unwinding.
            </p>
          </section>
        </>
      )}
