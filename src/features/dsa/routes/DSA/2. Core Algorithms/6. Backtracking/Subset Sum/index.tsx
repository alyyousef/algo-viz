...
const historicalMilestones = [
  ...
  },
  {
    ...
    detail: 'Recursive search made it possible to solve modest instances exactly.',
  },
  {
    title: 'Pseudo-polynomial DP (1970s)',
    detail: 'Dynamic programming offered a practical alternative when the target sum is small.',
  },
]
...
const mentalModels = [
  ...
  },
  {
    ...
    detail: 'Think of placing weights on a scale to hit a precise total.',
  },
  {
    title: 'Prune with bounds',
    detail: 'If the current sum exceeds the target or cannot reach it, stop exploring that branch.',
  },
]
...
const coreIdeas = [
  ...
  },
  {
    ...
    detail: 'You can stop at the first valid subset, or collect all subsets that meet the target.',
  },
  {
    title: 'Pruning',
    detail: 'Sort values and prune when the sum exceeds the target (for nonnegative inputs).',
  },
]
...
const dataStructures = [
  {
    title: 'Input array',
    detail: 'The list of numbers to consider, often sorted for pruning.',
  },
  {
    title: 'Current subset',
    detail: 'Stores the elements chosen so far if you need to output the subset.',
  },
  {
    title: 'Running sum',
    detail: 'Tracks the current total to compare against the target.',
  },
  {
    title: 'Index pointer',
    detail: 'Controls which elements are still available for selection.',
  },
]
...
const correctnessNotes = [
  ...
  {
    title: 'Completeness',
    detail: 'Backtracking explores every subset, so it finds a solution if one exists.',
  },
  {
    title: 'Soundness',
    detail: 'A solution is accepted only if its sum equals the target.',
  },
  {
    title: 'Pruning safety',
    detail: 'Pruning on sum > target is safe only when all remaining numbers are nonnegative.',
  },
]
...
const complexityNotes = [
  {
    ...
    detail: 'O(2^n) subsets in the worst case; backtracking is exponential.',
  },
  {
    title: 'Space complexity',
    detail: 'O(n) recursion depth plus the subset storage.',
  },
  {
    title: 'Pseudo-polynomial alternative',
    detail: 'DP can solve it in O(n * target) time when target is not too large.',
  },
  {
    title: 'Meet-in-the-middle',
    detail: 'Splitting the array yields O(2^(n/2)) time, faster for moderate n.',
  },
]
...
const edgeCases = [
  {
    title: 'Target is 0',
    detail: 'The empty subset is a valid solution.',
  },
  {
    title: 'Negative numbers',
    detail: 'Pruning on sum > target is not valid if negatives exist; branches may recover later.',
  },
  {
    title: 'Duplicates',
    detail:
...
const realWorldUses = [
  {
    ...
    detail: 'Choose items that sum exactly to a budget with discrete costs.',
  },
  {
    context: 'Cryptography',
    detail: 'Subset Sum underpins knapsack-based cryptosystems and hardness assumptions.',
  },
  {
    context: 'Load balancing',
    detail: 'Pick tasks that exactly fit a time or resource capacity window.',
  },
  {
    context: 'Test case generation',
    detail: 'Find subsets with specific totals to hit edge-case conditions.',
  },
  {
    context: 'Feature selection',
    detail: 'Select features whose costs or weights match a given threshold.',
  },
]
...
const examples = [
  {
    ...
    explanation: 'Each element is either included or excluded, forming a binary decision tree.',
  },
  {
...
target = 9
Solution subset: [4, 5]`,
    explanation: 'The backtracking search finds that 4 + 5 equals 9.',
  },
  {
...
Sort B sums and binary search for target - aSum.`,
    explanation: 'This reduces time from O(2^n) to roughly O(2^(n/2)).',
  },
]
...
const variants = [
  {
    ...
    detail: 'Collect every subset that hits the target rather than returning early.',
  },
  {
    title: 'Count subsets',
    detail: 'Count the number of subsets that sum to target instead of returning a boolean.',
  },
  {
    title: 'Bounded knapsack',
    detail: 'Allow each item to be used multiple times, leading to unbounded or bounded variants.',
  },
  {
    title: 'Approximate subset sum',
    detail: 'Approximation schemes trade accuracy for speed on large inputs.',
  },
]
...
export default function SubsetSumPage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Subset Sum</h1>
      <p>
        Subset Sum asks whether any subset of numbers adds up to a target. The backtracking solution
        branches on include vs exclude, pruning impossible paths and stopping when a valid subset is
        found.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              This problem captures the challenge of exact selection. Each element is either used or
              ignored, making the search space exponential. Backtracking keeps the approach simple
              while enabling pruning strategies.
            </p>
            {coreIdeas.map((item) => (
              <p key={item.title}>
      ...
      {activeTab === 'core-concepts' && (
        <>
          <section id="core-foundation" className="bin98-section">
            ...
            <p>
              Every subset corresponds to a unique path in the decision tree. By exploring both
              include and exclude branches, the algorithm is complete.
            </p>
            <p>
              The practical challenge is reducing wasted exploration. Sorting, tracking the running
              sum, and pruning invalid branches can make an exponential search much more manageable
              on modest inputs.
            </p>
          </section>
      ...
      {activeTab === 'examples' && (
        <>
          ...
          <section id="examples-notes" className="bin98-section">
            ...
            <p>
              The examples cover the core include or exclude recursion, a concrete instance with a
              matching subset, and the meet-in-the-middle technique used when plain backtracking
              becomes too expensive.
            </p>
            <p>
              Together they show the main tradeoff in this problem: exact correctness is
              straightforward, but scale depends heavily on pruning and alternative formulations.
            </p>
          </section>
        </>
      )}
