...
const historicalMilestones = [
  ...
  },
  {
    ...
    detail: 'Exact solvers improved by combining DP ideas with linear programming and pruning.',
  },
  {
...
const mentalModels = [
  {
    ...
    detail: 'A bitmask is a compact checklist. Bit i is 1 if city i has been visited, 0 otherwise.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Each mask represents a frontier of partial tours that share the same visited set.',
  },
]
...
const workflowSteps = [
  {
    ...
    detail: 'Decide whether you need a cycle, a path, or a fixed end.',
  },
  {
    title: 'Encode subsets as bitmasks',
    detail: 'Use n-bit masks; ensure the start bit is always included.',
  },
  {
    title: 'Fill dp by mask size',
    detail: 'Iterate masks in increasing size to ensure predecessors are ready.',
  },
  {
    title: 'Recover the tour',
    detail: 'Store parent pointers to rebuild the optimal order of cities.',
  },
]
...
const algorithmSteps = [
  {
    ...
    detail: 'Fix a start city, usually 0, and represent subsets as bitmasks over n cities.',
  },
  {
    title: 'Define DP state',
    detail: 'dp[mask][i] = minimum cost to start at 0, visit exactly mask, and end at city i.',
  },
  {
    title: 'Initialize base case',
    detail: 'dp[1 << 0][0] = 0, and all other states are INF.',
  },
  {
  ...
  },
  {
    ...
    detail: 'For fullMask, answer is min over i of dp[fullMask][i] + dist[i][0] (for cycle).',
  },
]
...
const implementationNotes = [
  ...
  {
    title: 'INF guard',
    detail: 'Use a large sentinel. If dp is INF, skip transitions to avoid overflow.',
  },
  {
    title: 'Parent reconstruction',
    detail: 'Store parent[mask][i] to rebuild the optimal tour in reverse.',
  },
  {
    title: 'Iterate by subset size',
    detail: 'Loop masks by popcount to ensure transitions use smaller subsets.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Precompute dist[i][j] for O(1) transitions. For Euclidean cases, compute once.',
  },
]
...
const optimizationNotes = [
  ...
  },
  {
    ...
    detail: 'Iterate masks in increasing order and keep dp in contiguous arrays for speed.',
  },
  {
    title: 'Symmetry reduction',
    detail: 'Fix the start city to remove rotational symmetry and reduce states by a factor of n.',
  },
]
...
const complexityNotes = [
  {
    title: 'Time cost',
    detail: 'Held-Karp runs in O(n^2 2^n), a big improvement over O(n!).',
  },
  {
    title: 'Space cost',
    detail: 'Memory is O(n 2^n) for the dp table.',
  },
  {
    title: 'Scaling limits',
    detail: 'In practice, exact bitmask DP handles about n <= 20 to 22 on typical hardware.',
  },
  {
    title: 'Heuristic alternatives',
    detail: 'For larger n, use 2-opt, simulated annealing, or Christofides for approximate tours.',
  },
]
...
const realWorldUses = [
  {
    ...
    detail: 'Small delivery or service routes can be solved exactly with bitmask DP.',
  },
  {
    context: 'PCB drilling and CNC paths',
    detail: 'Tool heads must visit a set of points once with minimal travel.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Ordering a small set of fragments by overlap can be modeled as TSP variants.',
  },
  {
    context: 'Batch job sequencing',
    detail: 'Small sets of tasks with setup costs can be optimized exactly.',
  },
  {
    context: 'Warehouse picking',
    detail: 'Optimal paths through a small number of pick locations when exactness matters.',
  },
]
...
const correctnessSketch = [
  {
    ...
    detail: 'An optimal tour contains an optimal prefix for its visited set and endpoint.',
  },
  {
    title: 'Subset completeness',
    detail: 'All subsets are enumerated, so the optimal tour cannot be missed.',
  },
  {
    title: 'Fixed start',
    detail: 'Pinning the start removes rotational duplicates without changing the optimum.',
  },
  {
    title: 'Return edge',
    detail: 'For cycles, the last step back to start completes the Hamiltonian tour.',
  },
]
...
tour.reverse()`,
    explanation: 'Track parent pointers to rebuild the optimal route after computing costs.',
  },
]
...
const advancedInsights = [
  ...
  },
  {
    ...
    detail: 'Iterate submasks efficiently with sub = (sub - 1) & mask to explore subsets.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Store parent pointers in dp to rebuild the optimal tour path after computing costs.',
  },
]
...
export default function BitmaskDPTSPPage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Bitmask DP (TSP)</h1>
      <p>
        Bitmask DP, also called the Held-Karp algorithm, solves the traveling salesman problem
        exactly by enumerating subsets of visited cities. It trades exponential time for optimality
        and is the go-to solution for small instances. This page explains the state, transitions,
        variants, and how to reconstruct the optimal tour.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              The key idea is to remember which cities have been visited (a bitmask) and where you
              are now (the endpoint). This captures enough information to extend the path optimally
              without rechecking all permutations.
            </p>
            <p>
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-implementation-notes" className="bin98-section">
          ...
          <section id="core-optimization" className="bin98-section">
            <h2 className="bin98-heading">Optimization Ideas</h2>
          ...
          <section id="core-implementation-checklist" className="bin98-section">
