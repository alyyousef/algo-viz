...
const formalDefinition = [
  {
    title: 'Inputs',
    detail: 'n items, each with weight w_i and value v_i. A capacity W limits total weight.',
  },
  {
    title: 'Decision',
    detail: 'Choose a subset of items (0/1) or a multiset (unbounded) to maximize total value.',
  },
  {
    title: 'Constraint',
    detail: 'Sum of chosen weights must be <= W. Items are indivisible in 0/1 knapsack.',
  },
  {
    title: 'Output',
    detail: 'The maximum achievable value, and optionally the list of chosen items.',
  },
]
...
const algorithmSteps = [
  {
    ...
    detail: 'Set dp[0][w] = 0 (no items). Decide if dp is item-major (i, w) or weight-only (w).',
  },
  {
    title: 'Choose transition',
    detail: 'Include or exclude item i, respecting capacity. Use the correct loop direction.',
  },
  {
    title: 'Fill DP table',
    detail: 'Iterate items then capacities. Each cell depends on smaller item counts.',
  },
  {
    title: 'Extract and rebuild',
    detail: 'Answer is dp[n][W]. Backtrack to recover the chosen items if needed.',
  },
]
...
const workedExample = [
  {
    ...
    detail: 'Items: (w, v) = (2, 6), (2, 10), (3, 12). Capacity W = 5.',
  },
  {
    title: '2D table idea',
    detail: 'dp has rows for items and columns for capacity 0..5. dp[i][w] stores best value.',
  },
  {
    title: 'Row updates',
    detail: 'After item 1: best is 6 at w >= 2. After item 2: best is 16 at w >= 4.',
  },
  {
    title: 'Final answer',
    detail: 'dp[3][5] = 22 by taking items (2, 10) and (3, 12).',
  },
]
...
const reconstructionSteps = [
  {
    ...
    detail: 'If dp[i][w] == dp[i-1][w], item i was not taken. Otherwise it was taken.',
  },
  {
    title: 'Move to previous state',
    detail: 'If taken, set w = w - w_i and continue with i-1.',
  },
  {
    title: 'Reverse the list',
    detail: 'You collect items from last to first. Reverse to output in original order.',
  },
]
...
const examples = [
  ...
  },
  {
    ...
    explanation: 'Compare dp[i][w] with dp[i-1][w]. If they differ, the item was taken.',
  },
  {
...
const advancedInsights = [
  ...
  },
  {
    ...
    detail: 'For multi-dimensional variants, keep only non-dominated states to limit growth.',
  },
]
...
const miniFaq = [
  ...
  },
  {
    ...
    answer: 'Only for fractional knapsack. For 0/1, greedy can be arbitrarily wrong.',
  },
  {
...
const glossary = [
  ...
  {
    term: 'Pseudo-polynomial',
    definition: 'Runtime polynomial in numeric W, not in input bit-length.',
  },
  {
    term: 'FPTAS',
    definition: 'Approximation scheme with controllable error and polynomial runtime.',
  },
  {
    term: 'Meet-in-the-middle',
    definition: 'Split items into halves and combine subset summaries in O(2^(n/2)).',
  },
  {
    term: 'Reconstruction',
    definition: 'Backtracking decisions to recover chosen items, not only final value.',
  },
]
...
export default function KnapsackPage(): JSX.Element {
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Knapsack Problem</h1>
      <p>
        The knapsack problem asks which items to pack when weight is scarce and value is the goal.
        This page covers the formal definition, classic recurrences, reconstruction, variant types,
        and practical performance trade-offs.
      </p>
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              Knapsack balances payoff against capacity. The core DP compares include vs exclude for
              each item and capacity. 0/1 forbids repeats, unbounded allows them. When W is too
              large, use meet-in-the-middle or approximation.
            </p>
          </section>
      ...
      {activeTab === 'core-concepts' && (
        <>
          <section id="core-variants" className="bin98-section">
            <h2 className="bin98-heading">Variants at a Glance</h2>
            {variants.map((block) => (
          ...
          <section id="core-mechanics" className="bin98-section">
            <h2 className="bin98-heading">How It Works: Mechanics</h2>
            {mechanics.map((block) => (
              <div key={block.heading}>
                <h3 className="bin98-subheading">{block.heading}</h3>
                <ul>
          ...
          <section id="core-reconstruction" className="bin98-section">
            <h2 className="bin98-heading">Reconstruction Checklist</h2>
          ...
          <section id="core-complexity" className="bin98-section">
            ...
            <p>
              O(nW) is fast when W is moderate, but it scales with the numeric capacity. If W is
              huge, consider value-based DP, meet-in-the-middle, bitsets, or approximation schemes.
            </p>
          </section>
