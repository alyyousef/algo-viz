...
const prerequisites = [
  {
    ...
    detail: 'Floyd-Warshall works on directed graphs and supports negative edges.',
  },
  {
    title: 'All-pairs objective',
    detail: 'You want shortest paths between every pair of nodes, not a single source.',
  },
  {
    title: 'No negative cycles',
    detail: 'If a negative cycle exists, shortest paths are undefined (but detectable).',
  },
  {
    title: 'Matrix representation',
    detail: 'The algorithm is matrix-based; O(V^2) memory is required.',
  },
]
...
const inputsOutputs = [
  {
    title: 'Input',
    detail: 'Graph G(V, E) with weights, or a prebuilt adjacency matrix.',
  },
  {
    title: 'Output',
    detail: 'Distance matrix for all pairs, plus optional next-hop for path reconstruction.',
  },
  {
    title: 'Optional',
    detail: 'Negative cycle indicator and reachability (via Warshall variant).',
  },
]
...
const formalDefinitions = [
  {
    title: 'DP state',
    detail: 'dist[k][i][j] is the shortest path from i to j using intermediates 1..k.',
  },
  {
    title: 'Recurrence',
    detail: 'dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]) for each k.',
  },
  {
    title: 'Initialization',
    detail: 'dist[i][i] = 0, dist[i][j] = w(i, j) if edge exists, else Infinity.',
  },
  {
    title: 'Negative cycle test',
    detail: 'If dist[v][v] < 0 after completion, a negative cycle is present.',
  },
]
...
const coreConcepts = [
  ...
  },
  {
    ...
    detail: 'If any dist[v][v] becomes negative after processing, a negative cycle is present.',
  },
]
...
const howItWorks = [
  {
    ...
    detail: 'Set dist[i][j] to edge weight, 0 on diagonal, and Infinity if no edge exists.',
  },
  {
    step: '2. Iterate over intermediates',
    detail: 'For each k, allow k as an intermediate and update all i, j pairs.',
  },
  {
    step: '3. Relax all pairs',
    detail: 'If dist[i][k] + dist[k][j] beats dist[i][j], replace it.',
  },
  {
    step: '4. Detect negative cycles',
    detail: 'After the loop, if any dist[v][v] < 0, shortest paths are undefined.',
  },
  {
    step: '5. Optional path reconstruction',
    detail: 'Maintain a next matrix to rebuild actual paths, not just distances.',
  },
]
...
const dataStructures = [
  {
    ...
    detail: 'Stores shortest distances between all pairs, updated in place.',
  },
  {
    title: 'Next matrix',
    detail: 'Stores the next hop to reconstruct a path between any i, j pair.',
  },
  {
    title: 'Infinity sentinel',
    detail: 'A large value marking missing edges; must guard against overflow on add.',
  },
  {
    title: 'Vertex ordering',
    detail: 'The k outer loop defines which intermediates are allowed at each stage.',
  },
]
...
const correctnessNotes = [
  {
    ...
    detail: 'After processing k, dist[i][j] is optimal using intermediates only from 1..k.',
  },
  {
    title: 'In-place safety',
    detail: 'Updates are safe because the recurrence only depends on k and prior values.',
  },
  {
    title: 'Negative cycle exposure',
    detail: 'Any cycle with negative total weight will make dist[v][v] negative.',
  },
]
...
const complexityNotes = [
  {
    ...
    detail: 'O(V^3) operations. Performance depends on cache locality and tight loops.',
  },
  {
    title: 'Space complexity',
    detail: 'O(V^2) memory for the distance matrix, plus optional O(V^2) for next matrix.',
  },
  {
    title: 'When it wins',
    detail: 'Small-to-medium dense graphs or when you need a complete distance matrix quickly.',
  },
  {
    title: 'When it loses',
    detail: 'Large sparse graphs. Johnson or repeated Dijkstra usually wins for E near V.',
  },
]
...
const realWorldUses = [
  {
    ...
    detail: 'Precompute shortest distances between every pair of routers in dense network cores.',
  },
  {
    context: 'Urban transit grids',
    detail: 'Small city networks with many transfers benefit from all-pairs distance matrices.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Static maps with small node counts can precompute all-pairs paths for instant lookup.',
  },
]
...
const examples = [
  ...
  },
  {
    ...
    return dist`,
    explanation: 'Use Infinity for missing edges. Multiple edges keep the minimum weight.',
  },
  {
    title: 'Path reconstruction',
        ...
        for i in 0..n-1:
            for j in 0..n-1:
                if dist[i][k] + dist[k][j] < dist[i][j]:
                    ...
                    next[i][j] = next[i][k]
    return next`,
    explanation: 'The next matrix stores the first hop. If next[i][j] is null, no path exists.',
  },
  {
...
const implementationNotes = [
  {
    ...
    detail: 'Skip relaxation when dist[i][k] or dist[k][j] is Infinity.',
  },
  {
    title: 'Path reconstruction',
    detail: 'If next[i][j] is null, no path exists; otherwise iterate next hops to build the path.',
  },
  {
    title: 'Memory planning',
    detail: 'V^2 space grows fast; consider Johnson for larger sparse graphs.',
  },
  {
    title: 'Loop order',
    detail: 'Use k outermost to maintain correctness; i,j inner loops aid cache locality.',
  },
]
...
const glossaryTerms = [
  {
    term: 'APSP',
    definition: 'All-pairs shortest paths: shortest distances between every source-target pair.',
  },
  { term: 'DP state', definition: 'Shortest distance with allowed intermediates up to index k.' },
  {
    term: 'Min-plus recurrence',
    definition: 'Update rule: dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]).',
  },
  {
    term: 'Distance matrix',
    definition: 'V x V table of current best path costs between all node pairs.',
  },
  { term: 'Next matrix', definition: 'First-hop table used to reconstruct actual shortest paths.' },
  {
    term: 'Infinity sentinel',
    definition: 'Large placeholder value used when no direct edge exists.',
  },
  {
    term: 'Negative cycle',
    definition: 'Cycle with total weight below zero; finite shortest paths are undefined.',
  },
  {
    term: 'Diagonal test',
    definition: 'If dist[v][v] < 0 after completion, a negative cycle exists.',
  },
  {
    term: 'Warshall variant',
    definition: 'Boolean closure form for reachability rather than weighted distances.',
  },
...
export default function FloydWarshallPage(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <p>
        Floyd-Warshall computes shortest paths between every pair of vertices by gradually allowing
        more intermediate nodes. It is compact, reliable, and easy to implement. With O(V^3) time
        and O(V^2) space, it shines on dense graphs and small networks where a full distance matrix
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-matrix" className="bin98-section">
            <h2 className="bin98-heading">Matrix Mechanics</h2>
          ...
          <section id="core-complexity" className="bin98-section">
            ...
            <p>
              Floyd-Warshall is the APSP workhorse when the graph is dense or the vertex count is
              small enough to fit a full V^2 distance table in memory.
            </p>
            <ul>
