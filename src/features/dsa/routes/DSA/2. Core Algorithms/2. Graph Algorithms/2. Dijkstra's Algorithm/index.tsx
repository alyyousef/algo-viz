...
const problemPatterns = [
  {
    ...
    detail: 'Find cheapest routes when edges represent time, cost, or risk and are non-negative.',
  },
  {
...
const inputSensitivity = [
  {
    ...
    detail: 'Any negative edge breaks correctness; Dijkstra can return suboptimal paths.',
  },
  {
  ...
  {
    title: 'Sparse graphs',
    detail: 'Adjacency lists and heaps perform well; Dijkstra scales nearly linearly with edges.',
  },
  {
    title: 'Dense graphs',
    detail: 'Priority queues add overhead; O(V^2) array-based Dijkstra may be competitive.',
  },
]
...
const performanceProfile = [
  {
    ...
    detail: 'Most runtime is spent in pop-min and push; choose a heap with good constants.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Contiguous adjacency lists and compact node IDs reduce cache misses in large graphs.',
  },
  {
    title: 'Early exit',
    detail: 'If only one target is needed, exit on first pop of that node to save work.',
  },
]
...
const variantsAndTweaks = [
  ...
  },
  {
    ...
    detail: 'Bucket queues remove the log factor for small integer weights.',
  },
  {
    title: 'Radix heap',
    detail: 'Exploits monotone keys to speed up integer-weight graphs.',
  },
  {
    title: 'A* overlay',
    detail: 'Add a heuristic to focus search toward a goal while preserving optimality.',
  },
]
...
const implementationTips = [
  {
    ...
    detail: 'Use 64-bit integers or bigints if weights and paths can be large.',
  },
  {
    title: 'Use adjacency lists',
    detail: 'They avoid O(V^2) scans and keep runtime proportional to edges.',
  },
  {
    title: 'Prefer early exit',
    detail: 'If you only need one destination, stop at the first pop of that node.',
  },
  {
    title: 'Handle stale heap entries',
    detail: 'Skip nodes whose popped distance no longer matches dist[u].',
  },
  {
    title: 'Store parents for paths',
    detail: 'Capture parent pointers on relaxation so paths are available without extra passes.',
  },
]
...
const glossary = [
  {
    ...
    definition: 'Single-source shortest-path algorithm for non-negative edge weights.',
  },
  {
    term: 'Relaxation',
    definition:
      'Updating a neighbor distance when a shorter path through the current node is found.',
  },
  {
    term: 'Tentative distance',
    definition: 'Current best-known upper bound on shortest path from source to a node.',
  },
  {
    term: 'Settled node',
    definition: 'A node whose shortest path is finalized at extract-min time.',
  },
  {
    term: 'Decrease-key',
    definition: 'Priority queue operation to lower the key for an existing node.',
  },
  {
    term: 'Stale heap entry',
    definition: 'An outdated queue record that no longer matches current best distance.',
  },
  {
    term: 'Shortest-path tree',
    definition:
      'Parent-pointer tree describing one optimal route from source to each reachable node.',
  },
  {
    term: "Dial's buckets",
    definition: 'Bucket queue method for small integer weights that can remove the log factor.',
  },
]
...
export default function DijkstrasPage(): JSX.Element {
  const { activeTab, setActiveTab, handleMinimize } = useTopicTabs({
    tabs,
    pageTitle: 'Dijkstra&apos;s Algorithm',
  ...
  return (
    ...
    >
      <h1 className="bin98-doc-title">Dijkstra&apos;s Algorithm</h1>
      <p>
        Dijkstra&apos;s algorithm is weighted BFS with discipline. It settles nodes in order of
        increasing cost using a priority queue, guaranteeing optimal paths in graphs whose edges
        never go negative. From road networks to link-state routers, it is the reliable workhorse
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              The algorithm asks: what is the cheapest way to reach every node when edges are
              non-negative? By always expanding the lowest-cost unsettled node and relaxing its
              outgoing edges, it builds a shortest-path tree without backtracking. The only risk is
              memory and heap overhead, not correctness, as long as weights stay non-negative.
            </p>
      ...
      {activeTab === 'core-concepts' && (
        <>
          ...
          <section id="core-complexity" className="bin98-section">
            ...
            <p>
              Implementation choices dominate real speed: heaps balance clarity and efficiency,
              buckets win when weights are tiny integers, and cache-aware layouts matter on
              billion-edge graphs more than minor asymptotic differences.
            </p>
          </section>
