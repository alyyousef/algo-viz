...
const mentalModels = [
  ...
  {
    title: 'Monotone gate',
    detail: 'Binary search works whenever a predicate flips from false to true exactly once.',
  },
  {
    title: 'Fence posts',
    detail: 'Think in terms of boundaries: find the first index where the condition becomes true.',
  },
]
...
const terminology = [
  {
    term: 'Lower bound',
    detail: 'First index i where a[i] >= target (or predicate becomes true).',
  },
  {
    term: 'Upper bound',
    detail: 'First index i where a[i] > target; last equal is upper-1.',
  },
  {
    term: 'Invariant',
    detail: 'A property maintained each loop, e.g. target is in [lo, hi].',
  },
  {
    term: 'Monotone predicate',
    detail: 'A boolean function that is false then true as input increases.',
  },
  {
    term: 'Feasible region',
    detail: 'The set of inputs where the predicate is true; binary search finds its boundary.',
  },
]
...
const stepByStep = [
  {
    ...
    detail: 'Decide whether the answer is always in [lo, hi] or [lo, hi). Stick to it.',
  },
  {
    title: 'Compute mid safely',
    detail: 'Use mid = lo + (hi - lo) // 2 to prevent overflow.',
  },
  {
    title: 'Shrink the interval',
    detail: 'If mid is too small, move lo up. If mid is large enough, move hi down.',
  },
  {
    title: 'Terminate correctly',
    detail: 'Stop when lo meets hi (for boundaries) or when lo > hi (for exact search).',
  },
]
...
const invariants = [
  ...
  },
  {
    ...
    detail: 'Each iteration must strictly reduce the search space to guarantee termination.',
  },
  {
    title: 'Consistent bounds',
    detail: 'Choose [lo, hi] or [lo, hi) and stick with it to avoid off-by-one bugs.',
  },
]
...
const alternatives = [
  {
    title: 'Linear search',
    detail: 'O(n) time but works on unsorted data and has lower constant overhead.',
  },
  {
    title: 'Hash tables',
    detail: 'Average O(1) lookup on unordered data; no ordering or range queries.',
  },
  {
    title: 'Balanced BSTs',
    detail: 'O(log n) lookup with dynamic inserts/deletes and ordered iteration.',
  },
  {
    title: 'Interpolation search',
    detail: 'Faster on uniformly distributed data but can degrade to O(n).',
  },
  {
    title: 'Exponential search',
    detail: 'Find a range quickly then binary search inside; great for unbounded arrays.',
  },
  {
    title: 'Ternary search',
    detail: 'For unimodal functions, not for sorted discrete arrays.',
  },
]
...
const numericBinarySearchNotes = [
  {
    ...
    detail: 'For integers, stop at lo == hi. For doubles, iterate until epsilon.',
  },
  {
    title: 'Predicate costs',
    detail: 'If predicate is O(n), the total cost is O(n log range).',
  },
  {
    title: 'Upper bounds',
    detail: 'Pick a safe high bound or expand exponentially until feasible.',
  },
]
...
const complexityNotes = [
  {
    ...
    detail: 'O(log n) comparisons. For a million items, about 20 steps.',
  },
  {
    title: 'Space complexity',
    detail: 'O(1) for iterative search. Recursive versions use O(log n) call stack.',
  },
  {
    title: 'Constant factors',
    detail: 'Branch prediction and cache behavior matter. Iterative loops are usually faster.',
  },
]
...
const useCases = [
  {
    ...
    detail: 'Fast membership checks, index lookup, or position for insertion.',
  },
  {
    context: 'First/last occurrence',
    detail: 'Find boundaries for duplicates or counts in a sorted list.',
  },
  {
    context: 'Search on answer',
    detail: 'Minimize capacity, time, or threshold with a monotone predicate.',
  },
  {
    context: 'Database indexing',
    detail: 'B-trees and sorted pages use binary search internally for key lookup.',
  },
  {
    context: 'Numeric root finding',
    detail: 'Binary search on a monotone function to find thresholds or breakpoints.',
  },
]
...
const examples = [
  {
    ...
    return -1`,
    explanation: 'Use lo + (hi - lo) / 2 to avoid overflow in languages with fixed int sizes.',
  },
  {
    ...
    while lo < hi:
        ...
        if arr[mid] < target:
            ...
            hi = mid
    return lo`,
    explanation: 'Lower bound returns arr.length if no element is >= target.',
  },
  {
...
answer = lo`,
    explanation: 'Feasibility is monotone: if capacity works, any larger capacity also works.',
  },
  {
  ...
  },
  {
    ...
    while lo < hi:
        ...
        if arr[mid] <= target:
            ...
            hi = mid
    return lo`,
    explanation: 'Use upperBound to count occurrences: count = upperBound - lowerBound.',
  },
  {
    ...
    return hi`,
    explanation: 'Stop after a fixed number of iterations or when hi - lo <= epsilon.',
  },
]
...
const advancedInsights = [
  {
    ...
    detail: 'Frame problems as: find smallest x such that predicate(x) is true.',
  },
  {
    title: 'Invariant-first reasoning',
    detail: 'Write invariants before coding; they prevent off-by-one bugs.',
  },
  {
  ...
  },
  {
    ...
    detail: 'Use binary lifting when searching for the first index where a prefix condition holds.',
  },
  {
...
export default function BinarySearchPage(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <p>
        Binary search is the standard way to locate values in sorted data and to solve problems with
        monotone predicates. It repeatedly halves the remaining interval, giving O(log n) time with
        tiny constants. The trick is managing boundaries and invariants to avoid off-by-one errors.
      </p>
      ...
      {activeTab === 'big-picture' && (
        <>
          <section id="bp-overview" className="bin98-section">
            ...
            <p>
              Binary search turns ordering into speed. With a sorted list, you can rule out half the
              elements on every comparison.
            </p>
            <p>
              The same idea applies to any monotone condition: if a test is false before some point
              and true after, binary search finds the boundary quickly.
            </p>
          </section>
        ...
        <>
          ...
          <section id="core-alternatives" className="bin98-section">
            ...
            {comparisonGrid.map((row) => (
              <p key={row.feature}>
                <strong>{row.feature}:</strong> Binary Search {row.binary}; Linear Search{' '}
                {row.linear}; Hash Table {row.hash}; Balanced BST {row.bst}.
              </p>
            ))}
          </section>
