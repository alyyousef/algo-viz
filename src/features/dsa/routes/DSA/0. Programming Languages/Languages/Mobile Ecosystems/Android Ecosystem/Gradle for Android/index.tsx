...
.gradle-android-help-tab-active {
  position: relative;
  top: 1px;
  background: #ffffff;
}

.gradle-android-help-main {
  flex: 1;
  min-height: 0;
  display: grid;
...
.gradle-android-help-toc-title {
  margin: 0 0 10px;
  font-size: 12px;
  font-weight: 700;
}

.gradle-android-help-toc-list {
  margin: 0;
  padding: 0;
  list-style: none;
}

.gradle-android-help-toc-item {
  margin: 0 0 8px;
}

.gradle-android-help-toc-link {
  color: #000;
  text-decoration: none;
  font-size: 12px;
}

.gradle-android-help-content {
  overflow: auto;
  padding: 14px 20px 20px;
}

.gradle-android-help-doc-title {
  margin: 0 0 10px;
  font-size: 20px;
  font-weight: 700;
}

.gradle-android-help-section {
  margin: 0 0 20px;
}

.gradle-android-help-heading {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 700;
}

.gradle-android-help-content p,
.gradle-android-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
.gradle-android-help-content ul {
  margin: 0 0 10px 20px;
  padding: 0;
}

.gradle-android-help-divider {
  margin: 14px 0;
  border: 0;
  border-top: 1px solid #d0d0d0;
}

.gradle-android-help-codebox {
  margin: 6px 0 10px;
  padding: 8px;
  background: #f4f4f4;
...
.gradle-android-help-codebox code {
  display: block;
...
function renderContentSection(section: ContentSection, isLast: boolean): JSX.Element {
  return (
    <section key={section.id} id={section.id} className="gradle-android-help-section">
      <h2 className="gradle-android-help-heading">{section.title}</h2>
      ...
      {isLast ? null : <hr className="gradle-android-help-divider" />}
    </section>
  )
}
...
function renderExampleSection(section: ExampleSection, isLast: boolean): JSX.Element {
  return (
    <section key={section.id} id={section.id} className="gradle-android-help-section">
      ...
      <div className="gradle-android-help-codebox">
...
export default function GradleForAndroidPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Gradle for Android (${activeTabLabel})`
  ...
  return (
    <div className="gradle-android-help-page">
      <style>{pageStyles}</style>
      <div className="gradle-android-help-window" role="presentation">
        <header className="gradle-android-help-titlebar">
          <span className="gradle-android-help-titletext">Gradle for Android</span>
          <div className="gradle-android-help-controls">
            <button
              className="gradle-android-help-control"
              type="button"
              aria-label="Minimize"
            ...
            <Link to="/algoViz" className="gradle-android-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="gradle-android-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`gradle-android-help-tab ${activeTab === tab.id ? 'gradle-android-help-tab-active' : ''}`}
        ...
        <div className="gradle-android-help-main">
          <aside className="gradle-android-help-toc" aria-label="Table of contents">
            <h2 className="gradle-android-help-toc-title">Contents</h2>
            <ul className="gradle-android-help-toc-list">
              {sectionLinks[activeTab].map((section) => (
                <li key={section.id} className="gradle-android-help-toc-item">
                  <a href={`#${section.id}`} className="gradle-android-help-toc-link">
                    {section.label}
                  </a>
                </li>
              ))}
          ...
          <main className="gradle-android-help-content">
            <h1 className="gradle-android-help-doc-title">Gradle for Android</h1>
