...
then Unreal is often the easier default`,
  ...
  },
  {
    ...
    description: [
      'Engine decisions are often really team decisions expressed through technology.',
    ],
...
const glossarySections: GlossarySection[] = [
  {
    ...
    title: 'Unity Terms',
    terms: [
      { term: 'GameObject', definition: 'A core Unity scene object that can hold components and participate in the hierarchy.' },
      { term: 'Component', definition: 'A behavior or data unit attached to a GameObject, such as a transform, renderer, or script.' },
      { term: 'Prefab', definition: 'A reusable authored object template that can be instantiated across scenes.' },
      { term: 'MonoBehaviour', definition: 'A common Unity base class for gameplay scripts tied to engine lifecycle callbacks.' },
      { term: 'Package Manager', definition: 'Unity tooling for installing and versioning engine features and ecosystem packages.' },
      { term: 'URP', definition: 'Universal Render Pipeline, a Unity render pipeline aimed at broad performance and platform coverage.' },
      { term: 'HDRP', definition: 'High Definition Render Pipeline, a Unity render pipeline for higher-end visual targets.' },
      { term: 'IL2CPP', definition: 'A Unity scripting backend that converts managed code to C++ for ahead-of-time compilation.' },
    ],
  },
  {
    id: 'glossary-unreal',
    ...
    terms: [
      { term: 'Actor', definition: 'A core Unreal gameplay object that can be placed in a level and extended through components or subclasses.' },
      { term: 'Blueprint', definition: 'Unreal visual scripting and class-authoring system used heavily by designers and technical teams.' },
      { term: 'Pawn', definition: 'An Unreal actor type intended to be possessed or controlled by players or AI.' },
      { term: 'Game Mode', definition: 'An Unreal framework class that defines high-level game rules and session behavior.' },
      { term: 'Nanite', definition: 'A modern Unreal virtualized geometry system associated with high-detail rendering workflows.' },
      { term: 'Lumen', definition: 'A modern Unreal global illumination and reflections system for dynamic lighting workflows.' },
      { term: 'World Partition', definition: 'An Unreal world-management system for large environments and streaming workflows.' },
      { term: 'Replication', definition: 'Unreal networking behavior for synchronizing state across server and clients.' },
    ],
  },
  {
    id: 'glossary-shared',
    ...
    terms: [
      { term: 'Scene / Level', definition: 'An authored runtime environment containing placed objects, assets, and gameplay context.' },
      { term: 'Render Pipeline', definition: 'The sequence of rendering stages and rules used to draw the frame.' },
      { term: 'Asset Pipeline', definition: 'The import, processing, organization, and runtime preparation path for content such as models, textures, audio, and animations.' },
      { term: 'Serialization', definition: 'How engine data is stored to disk and reconstructed by the editor or runtime.' },
      { term: 'Build Pipeline', definition: 'The process used to package a project into a runnable player or shipped build.' },
      { term: 'Hot Reload / Live Iteration', definition: 'Workflow support for seeing logic or content changes quickly without a full manual rebuild cycle.' },
      { term: 'Large World Streaming', definition: 'Loading and unloading world data in pieces so large environments remain practical at runtime.' },
      { term: 'Technical Artist', definition: 'A hybrid role that bridges art production and engine/tooling implementation.' },
    ],
  },
]
...
.unity-unreal-help-tab-active {
  position: relative;
  top: 1px;
  background: #ffffff;
}

.unity-unreal-help-main {
  flex: 1;
  min-height: 0;
  display: grid;
...
.unity-unreal-help-toc-title {
  margin: 0 0 10px;
  font-size: 12px;
  font-weight: 700;
}

.unity-unreal-help-toc-list {
  margin: 0;
  padding: 0;
  list-style: none;
}

.unity-unreal-help-toc-item {
  margin: 0 0 8px;
}

.unity-unreal-help-toc-link {
  color: #000;
  text-decoration: none;
  font-size: 12px;
}

.unity-unreal-help-content {
  overflow: auto;
  padding: 14px 20px 20px;
}

.unity-unreal-help-doc-title {
  margin: 0 0 10px;
  font-size: 20px;
  font-weight: 700;
}

.unity-unreal-help-section {
  margin: 0 0 20px;
}

.unity-unreal-help-heading {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 700;
}

.unity-unreal-help-content p,
.unity-unreal-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
.unity-unreal-help-content ul {
  margin: 0 0 10px 20px;
  padding: 0;
}

.unity-unreal-help-divider {
  margin: 14px 0;
  border: 0;
  border-top: 1px solid #d0d0d0;
}

.unity-unreal-help-codebox {
  margin: 6px 0 10px;
  padding: 8px;
  background: #f4f4f4;
...
.unity-unreal-help-codebox code {
  display: block;
  white-space: pre;
...
export default function UnityVsUnrealEnginePage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Unity vs Unreal Engine (${activeTabLabel})`
  ...
  return (
    <div className="unity-unreal-help-page">
      <style>{pageStyles}</style>
      <div className="unity-unreal-help-window" role="presentation">
        <header className="unity-unreal-help-titlebar">
          <span className="unity-unreal-help-titletext">Unity vs Unreal Engine</span>
          <div className="unity-unreal-help-controls">
            <button
              className="unity-unreal-help-control"
              type="button"
              aria-label="Minimize"
            ...
            <Link to="/algoViz" className="unity-unreal-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="unity-unreal-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`unity-unreal-help-tab ${activeTab === tab.id ? 'unity-unreal-help-tab-active' : ''}`}
        ...
        <div className="unity-unreal-help-main">
          <aside className="unity-unreal-help-toc" aria-label="Table of contents">
            <h2 className="unity-unreal-help-toc-title">Contents</h2>
            <ul className="unity-unreal-help-toc-list">
              {sectionLinks[activeTab].map((section) => (
                <li key={section.id} className="unity-unreal-help-toc-item">
                  <a href={`#${section.id}`} className="unity-unreal-help-toc-link">
                    {section.label}
                  </a>
                </li>
              ))}
          ...
          <main className="unity-unreal-help-content">
            <h1 className="unity-unreal-help-doc-title">Unity vs Unreal Engine</h1>
