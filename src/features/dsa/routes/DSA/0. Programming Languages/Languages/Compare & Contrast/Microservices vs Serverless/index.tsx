...
.micro-serverless-help-titlebar {
  position: relative;
  display: flex;
  align-items: center;
  padding: 2px 4px;
...
.micro-serverless-help-titletext {
  position: absolute;
  left: 50%;
...
.micro-serverless-help-controls {
  display: flex;
  gap: 2px;
  margin-left: auto;
}

.micro-serverless-help-control {
  width: 18px;
  height: 16px;
  padding: 0;
...
.micro-serverless-help-tab-active {
  background: #fff;
  position: relative;
  top: 1px;
}

.micro-serverless-help-main {
  flex: 1;
  min-height: 0;
  display: grid;
...
.micro-serverless-help-toc-title {
  margin: 0 0 10px;
  font-size: 12px;
  font-weight: 700;
}

.micro-serverless-help-toc-list {
  margin: 0;
  padding: 0;
  list-style: none;
}

.micro-serverless-help-toc-item {
  margin: 0 0 8px;
}

.micro-serverless-help-toc-link {
  color: #000;
  text-decoration: none;
  font-size: 12px;
}

.micro-serverless-help-content {
  overflow: auto;
  padding: 14px 20px 20px;
}

.micro-serverless-help-doc-title {
  margin: 0 0 10px;
  font-size: 20px;
  font-weight: 700;
}

.micro-serverless-help-section {
  margin: 0 0 20px;
}

.micro-serverless-help-heading {
  margin: 0 0 8px;
  font-size: 16px;
  font-weight: 700;
}

.micro-serverless-help-content p,
.micro-serverless-help-content li {
  font-size: 12px;
  line-height: 1.5;
}
...
.micro-serverless-help-content ul {
  margin: 0 0 10px 20px;
  padding: 0;
}

.micro-serverless-help-divider {
  margin: 14px 0;
  border: 0;
  border-top: 1px solid #d0d0d0;
}

.micro-serverless-help-codebox {
  margin: 6px 0 10px;
  padding: 8px;
  background: #f4f4f4;
...
.micro-serverless-help-codebox code {
  display: block;
  white-space: pre;
...
export default function MicroservicesVsServerlessPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.title = `Microservices vs Serverless (${activeTabLabel})`
  ...
  return (
    <div className="micro-serverless-help-page">
      <style>{pageStyles}</style>
      <div className="micro-serverless-help-window" role="presentation">
        <header className="micro-serverless-help-titlebar">
          <span className="micro-serverless-help-titletext">Microservices vs Serverless</span>
          <div className="micro-serverless-help-controls">
            <button
              className="micro-serverless-help-control"
              type="button"
              aria-label="Minimize"
            ...
            <Link to="/algoViz" className="micro-serverless-help-control" aria-label="Close">
              X
            </Link>
          </div>
        </header>

        <div className="micro-serverless-help-tabs" role="tablist" aria-label="Sections">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`micro-serverless-help-tab ${activeTab === tab.id ? 'micro-serverless-help-tab-active' : ''}`}
        ...
        <div className="micro-serverless-help-main">
          <aside className="micro-serverless-help-toc" aria-label="Table of contents">
            <h2 className="micro-serverless-help-toc-title">Contents</h2>
            <ul className="micro-serverless-help-toc-list">
              {sectionLinks[activeTab].map((section) => (
                <li key={section.id} className="micro-serverless-help-toc-item">
                  <a href={`#${section.id}`} className="micro-serverless-help-toc-link">
                    {section.label}
                  </a>
                </li>
              ))}
          ...
          <main className="micro-serverless-help-content">
            <h1 className="micro-serverless-help-doc-title">Microservices vs Serverless</h1>
