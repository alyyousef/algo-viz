...
export interface Menu97ItemProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  children: ReactNode
}
...
export const Menu97Item = forwardRef(function Menu97Item(
  ...
  return (
    <button ref={ref} type={type} className={itemClassName} role="menuitem" {...rest}>
      {children}
    </button>
  )
})
...
export const Menu97Dropdown = forwardRef(function Menu97Dropdown(
  ...
  const dropdownClassName = ['menu-97__dropdown', 'bevel-out', className].filter(Boolean).join(' ')
...
export interface Menu97DropdownItemProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  shortcut?: ReactNode
  children: ReactNode
}

export const Menu97DropdownItem = forwardRef(function Menu97DropdownItem(
  { className, children, shortcut, type = 'button', ...rest }: Menu97DropdownItemProps,
  ref: Ref<HTMLButtonElement>,
) {
  const dropdownItemClass = ['menu-97__dropdown-item', className].filter(Boolean).join(' ')

  return (
    <button ref={ref} type={type} className={dropdownItemClass} role="menuitem" {...rest}>
...
export const Menu97Separator: React.FC<Menu97SeparatorProps> = ({ className }) => (
  <div role="separator" className={['menu-97__separator', className].filter(Boolean).join(' ')} />
)
